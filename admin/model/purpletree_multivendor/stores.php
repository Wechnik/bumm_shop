<?php
class ModelPurpletreeMultivendorStores extends Model{
	    //seller area
		...
		public function getTotalStores($data = array()) { 
			$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer c JOIN " . DB_PREFIX . "purpletree_vendor_stores pvs ON(pvs.seller_id=c.customer_id)";
		...
		public function editStore($store_id,$data,$file = '',$admin_store_id){
			...
			if($this->config->get('purpletree_multivendor_multi_store')){	
		...
		// Assign store to product
			...
			if(!empty($data['seller_amount'])){
				$this->db->query("INSERT INTO " . DB_PREFIX . "purpletree_vendor_payments SET seller_id='".(int)$data['seller_id']."',transaction_id='".$this->db->escape($data['seller_transaction'])."',amount ='".$this->db->escape($data['seller_amount'])."', payment_mode ='".$this->db->escape($data['seller_payment'])."', status ='success' , created_at =NOW(), updated_at =NOW()");
			}
			if ($data['store_seo']) {
		...
		public function getTotalSellerOrders($data= array()){
			...
			if(!isset($data['filter_date_from']) && !isset($data['filter_date_to'])){
				...
				$sql .= " AND DATE(o.date_added) <= '".$this->db->escape(date('Y-m-d'))."'";
			}
