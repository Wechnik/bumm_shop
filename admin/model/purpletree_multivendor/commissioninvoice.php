<?php
class ModelPurpletreeMultivendorCommissioninvoice extends Model{
	   ...
	   public function getOrderProducts($commisionid,$order_id,$product_id,$seller_id){
		...
		public function getTotalCommissionsinvoices($data=array()){
			...
			$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "purpletree_vendor_commission_invoice pvc LEFT JOIN ". DB_PREFIX ."purpletree_vendor_commission_invoice_items pvcii ON (pvc.id = pvcii.link_id )LEFT JOIN ". DB_PREFIX ."purpletree_vendor_stores pvs ON (pvcii.seller_id = pvs.seller_id ) WHERE pvc.created_at != ''";
