<?php
class ModelPurpletreeMultivendorSubcriptionplan extends Model {
		public function addSubscriptionPlan($data) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "purpletree_vendor_plan SET no_of_product = '" . (int)$data['no_of_product'] . "', `joining_fee` = '" . (float)$data['joining_fee'] . "',
			`featured_store` = '" . (int)$data['featured_store'] . "', no_of_featured_product = '" . (int)$data['no_of_featured_product'] . "', no_of_category_featured_product = '" . (int)$data['no_of_category_featured_product'] . "',subscription_price = '" . (float)$data['subscription_price'] . "', validity = '" . (int)$data['validity'] . "',status = '" . (int)$data['status'] . "',default_subscription_plan = '" . (int)$data['default_subscription_plan'] . "', modified_date = NOW(), created_date = NOW()");
		...
		public function editSubscriptionPlan($plan_id,$data) {
			...
			$this->db->query("UPDATE " . DB_PREFIX . "purpletree_vendor_plan SET no_of_product = '" . (int)$data['no_of_product'] . "',featured_store = '" . (int)$data['featured_store'] . "', no_of_featured_product = '" . (int)$data['no_of_featured_product'] . "',no_of_category_featured_product = '" . (int)$data['no_of_category_featured_product'] . "', joining_fee = '" . (float)$data['joining_fee'] . "', subscription_price = '" . (float)$data['subscription_price'] . "', validity = '" . (int)$data['validity'] . "',status = '" . (int)$data['status'] . "',default_subscription_plan = '" . (int)$data['default_subscription_plan'] . "', modified_date = NOW() WHERE plan_id='". (int)$plan_id."'");
			//$plan_id = $this->db->getLastId();
			if($data['default_subscription_plan']==1){
				$this->db->query("UPDATE " . DB_PREFIX . "purpletree_vendor_plan SET default_subscription_plan = '0' WHERE plan_id != '".(int)$plan_id."'");
			}
