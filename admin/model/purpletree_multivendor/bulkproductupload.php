<?php
class ModelPurpletreeMultivendorBulkproductupload extends Model{
	...
	public function checkExistingProductIds($product_column,$value) {
		...
		public function getOptionValueId($name,$option_id){
			...
			$query = $this->db->query("SELECT option_value_id as opt_value_id FROM " . DB_PREFIX . "option_value_description WHERE name='".$this->db->escape(trim($name))."' and option_id='".$this->db->escape(trim($option_id))."' ");
			if ($query->num_rows) {
		...
		public function getExportSeoUrlData($tableName,$seller_id,$language){
			if($seller_id!=''){
				...
				$query1=$this->db->query("SELECT * FROM " . DB_PREFIX . "".$tableName."");
				$data=array();
		...
		public function getIsApproved($key='purpletree_multivendor_product_approval', $store_id = 0) {
