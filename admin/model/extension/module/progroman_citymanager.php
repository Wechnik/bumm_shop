...
class ModelExtensionModuleProgromanCityManager extends Model {
    ...
    public function getCustomerGroups() {
        return $this->db->query("SELECT * FROM prmn_cm_customer_group")->rows;
    }
    ...
    public function editCustomerGroups($data) {
        $query = "INSERT INTO prmn_cm_customer_group (`fias_id`, `customer_group_id`) VALUES\n";
        $values = [];

        foreach ($data as $row) {
            $values[] = "(" . (int)$row['fias_id'] . ", '" . (int)$row['customer_group_id'] . "')";
        }
    ...
    public function clearCustomeprGroups() {
        $this->db->query("TRUNCATE prmn_cm_customer_group");
    }
    ...
    private function installPrmnTables() {
        ...
        $this->db->query("
            CREATE TABLE IF NOT EXISTS `prmn_cm_customer_group` (
              `fias_id` INT(11) NOT NULL, 
    ...
    private function installFias() {
        ...
        if (!empty($this->db->query("SHOW TABLES LIKE 'fias'")->row)) {
            ...
            if (array_diff(['fias_id', 'parent_id', 'postalcode', 'offname', 'offname_gc', 'offname_pc', 'shortname', 'level', 'altnames'], $rows)) {
        ...
        $this->db->query("
            CREATE TABLE IF NOT EXISTS `fias` (
              ...
              `offname_gc` VARCHAR(120) DEFAULT NULL COMMENT 'Родительный падеж; Genetive',
              `offname_pc` VARCHAR(120) DEFAULT NULL COMMENT 'Предложный падеж; Prepositional',
    ...
    private function installSetting() {
        ...
        if (!$this->config->get('module_progroman_citymanager_setting')) {
            ...
            if ($setting) {
                ...
                $setting = [
                    'use_geoip' => 1,
                    'customer_group_for_new_user' => 1,
                    'customer_group_high_priority' => 1,
