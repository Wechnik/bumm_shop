      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-option" class="form-horizontal">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
              <select name="type" id="input-type" class="form-control">
                ...
                <?php if ($type == 'picture') { ?>
                <option value="picture" selected="selected">Изображение</option>
                <?php } else { ?>
                <option value="picture">Изображение</option>
                <?php } ?>
                    <?php if ($type == 'text') { ?>
          ...
          <table id="option-value" class="table table-striped table-bordered table-hover">
            <thead>
              <tr>
                <td class="text-left required" style="width: 200px;"><?php echo $entry_option_value; ?></td>
                <td class="text-left"><?php echo $entry_image; ?></td>
                <td class="text-right" style="width: 100px;">Цена опции</td>
                  <td class="text-left">+/-</td>
                <td class="text-right" style="width: 200px;">Товар из 1С:</td>
                <td class="text-right">Описание опции:</td>
                <td class="text-center" style="width: 1%;">Сорт.</td>
                <td></td>
              </tr>
            </thead>
            <tbody>
              ...
              <tr id="option-value-row<?php echo $option_value_row; ?>">
                ...
                <td class="text-left">
                  ...
                  <?php $selected_sync_name = false; ?>


                  <td class="text-right">
                      <input type="text" name="option_value[<?php echo $option_value_row; ?>][price]" value="<?php echo $option_value['price']; ?>" class="form-control" />
                  </td>

                  <td class="text-left increase-decrease">
                      <label class="radio-inline">
                          <?php if ($option_value['increase']) { ?>
                              <input type="radio" name="option_value[<?php echo $option_value_row; ?>][increase]" value="1" checked="checked" />
                              <span>+</span>
                          <?php } else { ?>
                              <input type="radio" name="option_value[<?php echo $option_value_row; ?>][increase]" value="1" />
                              <span>+</span>
                          <?php } ?>
                      </label>
                      <label class="radio-inline">
                          <?php if (!$option_value['increase']) { ?>
                              <input type="radio" name="option_value[<?php echo $option_value_row; ?>][increase]" value="0" checked="checked" />
                              <span>-</span>
                          <?php } else { ?>
                              <input type="radio" name="option_value[<?php echo $option_value_row; ?>][increase]" value="0" />
                              <span>-</span>
                          <?php } ?>
                      </label>
                  </td>
                  ...
                  <td class="text-center">
                      <div class="form-group">
                          <div class="col-sm-12 sync-code">
                              <select name="option_value[<?php echo $option_value_row; ?>][sync_code]" class="form-control">
                                  <option value="0" selected="selected"></option>
                                  <?php foreach($sync_products as $sync_product) { ?>
                                      <?php if($sync_product['code'] == $option_value['sync_code']) { ?>
                                          <option value="<?php echo $sync_product['data_code']; ?>" selected="selected"><?php echo $sync_product['name']; ?></option>
                                          <?php $selected_sync_name = $sync_product['name']; ?>
                                      <?php } else { ?>
                                          <option value="<?php echo $sync_product['data_code']; ?>"><?php echo $sync_product['name']; ?></option>
                                      <?php } ?>
                                  <?php } ?>
                              </select>
                                  <input type="hidden" name="option_value[<?php echo $option_value_row; ?>][sync_name]" value="<?php echo $selected_sync_name;  ?>" class="sync-name" >
                          </div>
                      </div>
                  </td>
                  <td class="text-center">
                      <div class="form-group">
                          <div class="col-sm-12">
                              <textarea name="option_value[<?php echo $option_value_row; ?>][option_description]" id="option_description<?php echo $option_value_row; ?>"
                                        placeholder="Описание опции" class="form-control" ><?php echo isset($option_value['option_description']) ? $option_value['option_description'] : false; ?>
                              </textarea>
                          </div>
                      </div>
                  </td>

                <td class="text-center"><input type="text" name="option_value[<?php echo $option_value_row; ?>][sort_order]" value="<?php echo $option_value['sort_order']; ?>" class="form-control" /></td>
                <td class="text-left"><button type="button" onclick="$('#option-value-row<?php echo $option_value_row; ?>').remove();" data-toggle="tooltip" title="<?php echo $button_remove; ?>" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>
                  ...
                  <script type="text/javascript">
                      ckeditorInit('option_description<?php echo $option_value_row; ?>', getURLVar('token'));
...
$('select[name=\'type\']').on('change', function() {
	if (this.value == 'select' || this.value == 'radio' || this.value == 'checkbox' || this.value == 'image' || this.value == 'picture' || this.value == 'text') {
		$('#option-value').show();
	} else {
		$('#option-value').hide();
	}
...
function addOptionValue() {
    ...
    html += '<input type="text" name="option_value[' + option_value_row + '][price]" value="" class="form-control" />';
    html += '</td>';

    html += '<td class="text-left increase-decrease">';

    html += '<label class="radio-inline">';
    html += '<input type="radio" name="option_value[' + option_value_row + '][increase]" value="1" checked="checked" />';
    html += '<span>+</span>';
    html += '</label>';

    html += '<label class="radio-inline">';
    html += '<input type="radio" name="option_value[' + option_value_row + '][increase]" value="0" checked="checked" />';
    html += '<span>-</span>';
    html += '</label>';
    ...
    html += '<select name="option_value[' + option_value_row + '][sync_code]" class="form-control">';
    html += '<option value="0" selected="selected"></option>';
        <?php foreach($sync_products as $sync_product) { ?>
    html += '<option value="<?php echo $sync_product['code']; ?>"><?php echo $sync_product['name']; ?></option>';
    <?php } ?>
    html += '</select>';
    html += '</div>';
    html += '</div>';
    ...
    html += '<div class="form-group">';
    html += '<div class="col-sm-12">';
    html += '<textarea name="option_value[' + option_value_row + '][option_description]" rows="3" id="option_description' + option_value_row + '" class="form-control">';
    html += '</textarea>';
    html += '</div>';
    html += '</div>';
    html += '</td>';
    ...
    ckeditorInit('option_description' + option_value_row, getURLVar('token'));

	option_value_row++;
}

$('.sync-code select').on('change', function(){
    $(this).siblings('.sync-name').val($(this).find('option:selected').text());
});

$( document ).ready(function() {
    $('.sync-code select').trigger('change');
});


//--></script></div>
    <style type="text/css">
        .increase-decrease .radio-inline {
            margin: 0;
            float: left;
        }
