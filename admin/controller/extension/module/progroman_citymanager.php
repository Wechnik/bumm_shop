...
class ControllerExtensionModuleProgromanCityManager extends Controller {
    ...
    public function index() {
        ...
        $data['url_customer_group'] = $this->makeModuleUrl('customergroup');
    ...
    public function customerGroup() {
        if (VERSION < '2.1') {
            $data['url_add_customer_group'] = $this->makeUrl('sale/customer_group/add');
            $this->load->model('sale/customer_group');
            $data['customer_groups'] = $this->model_sale_customer_group->getCustomerGroups();
        } else {
            $data['url_add_customer_group'] = $this->makeUrl('customer/customer_group/add');
            $this->load->model('customer/customer_group');
            $data['customer_groups'] = $this->model_customer_customer_group->getCustomerGroups();
        }

        $data['action_customer_group'] = $this->makeModuleUrl('savecustomergroups');
        $data['settings'] = $this->config->get('module_progroman_citymanager_setting');

        $data['cm_customer_groups'] = $this->model_citymanager->getCustomerGroups();
        foreach ($data['cm_customer_groups'] as & $group) {
            $group['fias_name'] = $this->model_citymanager->getFiasName($group['fias_id']);
        }

        $this->initLangVariables($data);

        $this->response->setOutput($this->loadView('customer_group', $data));
    }
    ...
    public function getSecretKey() {
        ...
        if ($this->validLicense()) {
            $json['message'] = $this->language->get('text_license_exists');
    ...
    public function saveCustomerGroups() {
        ...
        if ($this->hasPermission()) {
            if (isset($this->request->post['customer_groups'])) {
                ...
                if (empty($json['errors'])) {
                    $this->model_citymanager->clearCustomeprGroups();

                    if (!empty($this->request->post['customer_groups'])) {
                        $this->model_citymanager->editCustomerGroups($this->request->post['customer_groups']);
                    }
                }
            } else {
                $this->model_citymanager->clearCustomeprGroups();
