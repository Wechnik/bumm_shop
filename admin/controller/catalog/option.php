...
class ControllerCatalogOption extends Controller {
	...
	protected function getForm() {
        ...
        $tmp_sync_products = $this->model_catalog_sync_1c->getProducts();

        foreach ($tmp_sync_products as $sync_prod_k => $sync_prod_v) {

            $code_arr = [$sync_prod_v['code'], $sync_prod_v['availability_moscow'], $sync_prod_v['availability_novosibirsk']];

            $data['sync_products'][$sync_prod_k] = [
                'name' => $sync_prod_v['name'],
                'code' => $sync_prod_v['code'],
                'data_code' => implode(',' , $code_arr),
            ];
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			foreach ($options as $option) {
				...
				if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'image' || $option['type'] == 'text' || $option['type'] == 'picture') {
