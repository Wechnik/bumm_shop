...
class ControllerCatalogProduct extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['filter_meta_name'])) {
				$url .= '&filter_meta_name=' . urlencode(html_entity_decode($this->request->get['filter_meta_name'], ENT_QUOTES, 'UTF-8'));
			}
      ...
      if (isset($this->request->get['filter_category'])) {
        $url .= '&filter_category=' . $this->request->get['filter_category'];
            ...
            $this->model_catalog_product->search_index();
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_meta_name'])) {
			$filter_meta_name = $this->request->get['filter_meta_name'];
		} else {
			$filter_meta_name = null;
		}
		...
		$data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link($href_edit, 'token=' . $this->session->data['token'] . $url, true)
		);
      ...
      $category =  $this->model_catalog_product->getProductCategories($result['product_id']);
		...
		$data['filter_meta_name'] = $filter_meta_name;
    ...
    if ($this->config->get('config_editor_default')) {
        ...
        $data['entry_video_in_product'] = $this->language->get('entry_video_in_product');
		$data['entry_status'] = $this->language->get('entry_status');
        ...
        $data['entry_additional_description_1'] = $this->language->get('entry_additional_description_1');
        $data['entry_additional_description_2'] = $this->language->get('entry_additional_description_2');
        $data['help_additional_description_1'] = $this->language->get('help_additional_description_1');
        $data['help_additional_description_2'] = $this->language->get('help_additional_description_2');
		...
		if (isset($this->request->post['product_options_text'])) {
            $data['product_options_text'] = $this->request->post['product_options_text'];
		} elseif (!empty($product_info['product_options_text'])) {
			$data['product_options_text'] = $product_info['product_options_text'];
		} else {
			$data['product_options_text'] = '';
		}
        ...
        if (isset($this->request->post['product_stickers'])) {
            $data['current_product_stickers'] = $this->request->post['product_stickers'];
        } elseif (!empty($product_info)) {
            $data['current_product_stickers'] = json_decode($product_info['product_stickers'], true);
        } else {
        ...
        if (isset($this->request->post['product_attribute'])) {
            $data['smart_filters'] = $this->request->post['product_attribute'];
        } elseif (isset($this->request->get['product_id'])) {
            $data['smart_filters'] = $this->model_catalog_product->getSmartFilters($this->request->get['product_id']);
        } else {
            $data['smart_filters'] = array();
        }


        if (isset($this->request->post['universal_product'])) {
            $data['universal_product'] = $this->request->post['universal_product'];
        } elseif (!empty($product_info)) {
            $data['universal_product'] = $product_info['universal_product'];
        } else {
            $data['universal_product'] = 0;
        }


        if (isset($this->request->post['show_after_filtration'])) {
            $data['show_after_filtration'] = $this->request->post['show_after_filtration'];
        } elseif (!empty($product_info)) {
            $data['show_after_filtration'] = $product_info['show_after_filtration'];
        } else {
            $data['show_after_filtration'] = 0;
        }
		...
		$data['car_years'] = $this->model_smart_filter_car_year->getCarYears();
        ...
        $tmp_car_models = $this->model_smart_filter_car_model->getCarModels();

        if($tmp_car_models) {

            foreach ($tmp_car_models as $tmp_car_model_k => $tmp_car_model_v) {
                $data['car_models'][$tmp_car_model_v['id']] = $tmp_car_model_v;
            }
        ...
        $tmp_car_marks = $this->model_smart_filter_car_mark->getCarMarks();

        if($tmp_car_marks) {

            foreach ($tmp_car_marks as $tmp_car_mark_k => $tmp_car_mark_v) {
                $data['car_marks'][$tmp_car_mark_v['id']] = $tmp_car_mark_v;
            }
        ...
        $data['car_generations'] = [];

        $tmp_car_generations = $this->model_smart_filter_car_generation->getCarGenerations();

        if($tmp_car_generations) {

            foreach ($tmp_car_generations as $tmp_car_generation_k => $tmp_car_generation_v) {
                $data['car_generations'][$tmp_car_generation_v['id']] = $tmp_car_generation_v;
            }
		...
		if (isset($this->request->post['stat_seo_page'])) {
			$data['stat_seo_page'] = $this->request->post['stat_seo_page'];
		} elseif (!empty($product_info)) {
			$data['stat_seo_page'] = $product_info['stat_seo_page'];
		} else {
			$data['stat_seo_page'] = false;
		}
    ...
    $data['manufacturers'] = $this->model_catalog_manufacturer->getManufacturers();
		...
		foreach ($product_options as $product_option) {
			...
			if (isset($product_option['product_option_value'])) {
				foreach ($product_option['product_option_value'] as $product_option_value) {
                    ...
                    if (is_file(DIR_IMAGE . $product_option_value['product_option_image'])) {
                        $opt_image = $product_option_value['product_option_image'];
                        $opt_thumb = $product_option_value['product_option_image'];
                    } else {
                        $opt_image = '';
                        $opt_thumb = 'no_image.png';
                    }
					...
					$product_option_value_data[] = array(
						...
						'product_option_thumb'       => $this->model_tool_image->resize($opt_thumb, 100, 100),
		...
		if (isset($this->request->post['preview'])) {
			...
			$data['preview'] = $product_info['preview'];
		} else {
			$data['preview'] = '';
		}
		...
		if (isset($this->request->post['preview']) && is_file(DIR_IMAGE . $this->request->post['preview'])) {
			$data['thumb_preview'] = $this->model_tool_image->resize($this->request->post['preview'], 100, 100);
		} elseif (!empty($product_info) && is_file(DIR_IMAGE . $product_info['preview'])) {
			$data['thumb_preview'] = $this->model_tool_image->resize($product_info['preview'], 100, 100);
		} else {
        ...
        if (isset($this->request->post['video_in_product'])) {
            $data['video_in_product'] = $this->request->post['video_in_product'];
        } elseif (!empty($product_info)) {
            $data['video_in_product'] = $product_info['video_in_product'];
        } else {
            $data['video_in_product'] = '';
        }

        if (isset($this->request->post['video_in_product1'])) {
            $data['video_in_product1'] = $this->request->post['video_in_product1'];
        } elseif (!empty($product_info)) {
            $data['video_in_product1'] = $product_info['video_in_product1'];
        } else {
            $data['video_in_product1'] = '';
        }

        if (isset($this->request->post['video_in_product2'])) {
            $data['video_in_product2'] = $this->request->post['video_in_product2'];
        } elseif (!empty($product_info)) {
            $data['video_in_product2'] = $product_info['video_in_product2'];
        } else {
            $data['video_in_product2'] = '';
        }
		...
		foreach ($products as $product_id) {
			...
			if ($related_info) {
				$data['product_relateds'][] = array(
					...
					'name'       => $related_info['meta_h1']
				);
			}
		}

		if (isset($this->request->post['product_related_similar'])) {
			$products = $this->request->post['product_related_similar'];
		} elseif (isset($this->request->get['product_id'])) {
			$products = $this->model_catalog_product->getProductRelatedSimilar($this->request->get['product_id']);
		} else {
			$products = array();
		}

		$data['product_related_similars'] = array();
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model']) || isset($this->request->get['filter_meta_name'])) {
			$this->load->model('catalog/product');
			$this->load->model('catalog/option');
    ...
    public function smartFilterAutocomplete() {

        $json = array();

        if (isset($this->request->get['car_mark']) || isset($this->request->get['car_model']) || isset($this->request->get['car_generation'])) {
            $this->load->model('catalog/product');

            if (isset($this->request->get['car_mark'])) {
                $filter_car_mark = $this->request->get['car_mark'];
            } else {
                $filter_car_mark = '';
            }

            if (isset($this->request->get['car_model'])) {
                $filter_car_model = $this->request->get['car_model'];
            } else {
                $filter_car_model = '';
            }

            if (isset($this->request->get['car_generation'])) {
                $filter_car_generation = $this->request->get['car_generation'];
            } else {
                $filter_car_generation = '';
            }
            ...
            $filter_data = array(
                'filter_car_mark'  => $filter_car_mark,
                'filter_car_model' => $filter_car_model,
                'filter_car_generation' => $filter_car_generation,
                'start'        => 0,
                'limit'        => $limit,
                'field'     => $this->request->get['field']
            );

            $json = $this->model_catalog_product->getSmartFiltersAutocomplete($filter_data);
        }
    ...
    public function smartFilterGetModels() {
	    ...
	    if(isset($this->request->get['car_mark'])) {
	        ...
	        $mark_id = $this->request->get['car_mark'];

            $json = $this->model_catalog_product->getModelsByMark($mark_id);
    ...
    public function smartFilterGetGenerations() {
	    ...
	    if(isset($this->request->get['car_model'])) {
            ...
            $model_id = $this->request->get['car_model'];

            $json = $this->model_catalog_product->getGenerationsByModel($model_id);
