	...
	class ControllerCatalogCategory extends Controller {
		...
		public function add() {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
				...
				$this->model_extension_module_nkf_autocategory->edit($category_id, $this->request->post);
		...
		protected function getList() {
			...
			$data['sort_categories'] = $this->url->link('catalog/category/sort_categories', 'token=' . $this->session->data['token'] . $url, true);
		...
		protected function getForm() {
			...
			$data['tab_nkf_autocategory'] = $this->language->get('tab_nkf_autocategory');
			$data['entry_nkf_column_attribute_not'] = $this->language->get('entry_nkf_column_attribute_not');
			$data['entry_nkf_column_attribute'] = $this->language->get('entry_nkf_column_attribute');
			$data['entry_nkf_column_attribute_use_from_to'] = $this->language->get('entry_nkf_column_attribute_use_from_to');
			$data['entry_nkf_column_attribute_value'] = $this->language->get('entry_nkf_column_attribute_value');
			$data['entry_nkf_column_attribute_value_from'] = $this->language->get('entry_nkf_column_attribute_value_from');
			$data['entry_nkf_column_attribute_value_to'] = $this->language->get('entry_nkf_column_attribute_value_to');
			$data['entry_nkf_column_price_from'] = $this->language->get('entry_nkf_column_price_from');
			$data['entry_nkf_column_price_to'] = $this->language->get('entry_nkf_column_price_to');
			$data['entry_nkf_column_is_special'] = $this->language->get('entry_nkf_column_is_special');
			$data['entry_nkf_column_manufacturer'] = $this->language->get('entry_nkf_column_manufacturer');
			$data['entry_nkf_column_category'] = $this->language->get('entry_nkf_column_category');
			$data['entry_nkf_column_in_name'] = $this->language->get('entry_nkf_column_in_name');
			$data['entry_autocategory_settings'] = $this->language->get('entry_autocategory_settings');
			$data['text_no_matter'] = $this->language->get('text_no_matter');
			$data['button_remove'] = $this->language->get('button_remove');
			$data['text_yes'] = $this->language->get('text_yes');
			$data['text_no'] = $this->language->get('text_no');
			
			if (isset($this->request->get['category_id']))
				$data['nkf_category_id']=$this->request->get['category_id'];
			else
				$data['nkf_category_id']=0;
			
			$data['button_nkf_autocategory_add'] = $this->language->get('button_nkf_autocategory_add');
			$data['button_nkf_run'] = $this->language->get('button_nkf_run');
			
			$data['autocategory_settings']=$this->url->link('extension/module/nkf_autocategory', 'token=' . $this->session->data['token'], true);
			
			$data['nkf_autocategory']=array();
...
//			if (isset($this->request->post['user_groups_fields'])) {
//				$data['user_groups_fields'] = $this->request->post['user_groups_fields'];
//			} elseif (!empty($category_info['user_groups_fields'])) {
//				$data['user_groups_fields'] = json_decode($category_info['user_groups_fields'], true);
//			} else {
//				$data['user_groups_fields'] = [];
//			}
...
//			$data['user_groups_fields'] = json_decode($category_info['user_groups_fields'], true);
			...
			$data['field_list_arr'] = [
				...
				'price'	=>  'Цена',
				'quantity'	=>  'Количество',
				'sizes'	=>  'Размеры (Д x Ш x В)',
				'weight'	=>  'Вес',
				'manufacturer'	=>  'Производитель',
				'main_category'	=>  'Главная категория',
				'tab_text'	=>  'Текста для таба',
				'text_after_description'	=>  'Текст для после описания',
			];
			...
			if (isset($this->request->post['show_main_page'])) {
				$data['show_main_page'] = $this->request->post['show_main_page'];
			} elseif (!empty($category_info)) {
				$data['show_main_page'] = $category_info['show_main_page'];
			} else {
				$data['show_main_page'] = 0;
			}
			...
			if (isset($this->request->post['nkf_autocategory'])) {
				$data['nkf_autocategory'] = $this->request->post['nkf_autocategory'];
			} elseif (isset($this->request->get['category_id'])) {
				$this->load->model('extension/module/nkf_autocategory');
				$data['nkf_autocategory'] = $this->model_extension_module_nkf_autocategory->getAutoCategory($this->request->get['category_id']);
			} else {
				$data['nkf_autocategory'] = array();
			}
		...
		public function sort_categories()
		{
			...
			$data['categories'] = $this->model_catalog_category->getFilterSortCategories();
			
			
			
			if (($this->request->server['REQUEST_METHOD'] == 'POST') ) {
				$this->model_catalog_category->editFilterSortCategories($this->request->post);
			...
			$this->response->setOutput($this->load->view('catalog/sort_categories', $data));
		}
