...
class ControllerCommonHeader extends Controller {
	public function index() {
        ...
        $data['text_checks_left']       = $this->language->get('text_checks_left');
        $data['text_uniq_text']         = $this->language->get('text_uniq_text');
        $data['text_url']               = $this->language->get('text_url');
        $data['text_uniq']              = $this->language->get('text_uniq');
        $data['text_check_text']        = $this->language->get('text_check_text');
        $data['button_show_modal']      = $this->language->get('button_show_modal');
        $data['text_show_result']       = $this->language->get('text_show_result');
        $data['text_show_all_result']   = $this->language->get('text_show_all_result');
        $data['text_double_space']      = $this->language->get('text_double_space');
        $data['error_is_not_url']       = $this->language->get('error_is_not_url');
        $data['text_delete_double_space']       = $this->language->get('text_delete_double_space');
        $data['text_ignore_url']        = $this->language->get('text_ignore_url');
        $data['text_text_length']       = $this->language->get('text_text_length');
        $data['text_char_length']       = $this->language->get('text_char_length');
        $data['text_words_length']      = $this->language->get('text_words_length');

        $data['content_watch_range_0']          = $this->config->get('content_watch_range_0');
        $data['content_watch_range_1']          = $this->config->get('content_watch_range_1');
        $data['show_range']                     = $this->config->get('content_watch_show_range');
        ...
        if(!$this->config->get('content_watch_show_button')){
            if(isset($this->request->request['route']) AND in_array($this->request->request['route'],$_routers)){

                $data['content_watch_show_button'] =true;
            }else{
        ...
        if ($this->user->hasPermission('access','extension/feed/content_watch') AND $this->config->get('content_watch_status')) {
            $data['content_watch_permission'] = true;
        }else{
            $data['content_watch_permission'] = false;
        }
