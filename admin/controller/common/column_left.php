...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['token']) && isset($this->session->data['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
			...
			if ($this->user->hasPermission('access', 'catalog/tab')) {
				$catalog[] = array(
					'name'	   => $this->language->get('tab_product_tab'),
					'href'     => $this->url->link('catalog/tab', 'token=' . $this->session->data['token'], true),
					'children' => array()		
				);
			}
			...
			if ($this->user->hasPermission('access', 'catalog/sync_1c')) {
				$catalog[] = array(
					'name'	   => $this->language->get('text_sync_1c'),
					'href'     => $this->url->link('catalog/sync_1c', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}


			if ($this->user->hasPermission('access', 'catalog/custom_tabs')) {
				$catalog[] = array(
                    'name'	   => $this->language->get('Доп. вкладки(настройка)'),
					'href'     => $this->url->link('catalog/custom_tabs', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}
		 ...
		 /*	$smart_filter = array();
			...
			if ($this->user->hasPermission('access', 'smart_filter/car_year')) {
                $smart_filter[] = array(
					'name'	   => $this->language->get('text_car_year'),
					'href'     => $this->url->link('smart_filter/car_year', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}

			if ($this->user->hasPermission('access', 'smart_filter/car_mark')) {
                $smart_filter[] = array(
					'name'	   => $this->language->get('text_car_mark'),
					'href'     => $this->url->link('smart_filter/car_mark', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}

			if ($this->user->hasPermission('access', 'smart_filter/car_model')) {
                $smart_filter[] = array(
					'name'	   => $this->language->get('text_car_model'),
					'href'     => $this->url->link('smart_filter/car_model', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}

			if ($this->user->hasPermission('access', 'smart_filter/car_generation')) {
                $smart_filter[] = array(
					'name'	   => $this->language->get('text_car_generation'),
					'href'     => $this->url->link('smart_filter/car_generation', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}   */
		   ...
		   //	if ($smart_filter) {
				$data['menus'][] = array(
                    'id'       => 'menu-smart-filter',
                    'icon'	   => 'fa-filter',
                    'name'	   => $this->language->get('text_smart_filter'),
                    'href'     => $this->url->link('smart_filter/smart_filter', 'token=' . $this->session->data['token'], true),
                    'children' => []
				  //	'children' => $smart_filter
				);
		  //	}
