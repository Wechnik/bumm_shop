<?php
class ControllerPurpletreeMultivendorSubscriptionplanPendinginvoice extends Controller {
		private $error = array();
		...
		public function index() {
			...
			$this->load->model('purpletree_multivendor/subscriptionplanpendinginvoice');
		...
		public function getList() {
			...
			'href' => $this->url->link('purpletree_multivendor/subscriptionplan_pendinginvoice', 'token=' . $this->session->data['token'] . $url, true)
			);
			...
			$subs_invoice_total=$this->model_purpletree_multivendor_subscriptionplanpendinginvoice->getTotalSellerInvoice($filter_data);
			$results = $this->model_purpletree_multivendor_subscriptionplanpendinginvoice->getSellerInvoiceList($filter_data);
			if(!empty($results)){	
				foreach ($results as $result) {
					$subscription_status='';
					$s_status=$this->model_purpletree_multivendor_subscriptionplanpendinginvoice->getSubscriptionBySellerId($result['seller_id']);
					if($s_status==1){
			...
			$pagination->url = $this->url->link('purpletree_multivendor/subscriptionplan_pendinginvoice', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
			...
			$this->response->setOutput($this->load->view('purpletree_multivendor/subscriptionplan_pendinginvoice', $data));
		}
}