<?php
class ControllerPurpletreeMultivendorManagesubscriptionplan extends Controller {
		private $error = array();
		...
		public function index() {
			...
			$this->load->model('purpletree_multivendor/managesubscriptionplan');
			$this->getList();
		}
		...
		public function add() {
			...
			if (!$this->customer->validateSeller()) {
				$this->load->language('purpletree_multivendor/ptsmultivendor');
				$this->session->data['error_warning'] = $this->language->get('error_license');
				$this->response->redirect($this->url->link('purpletree_multivendor/managesubscriptionplan', 'token=' . $this->session->data['token'] . $url, true));
			}
			...
			if (!isset($this->request->get['seller_id']) || $this->request->get['seller_id'] == '') {
				$this->response->redirect($this->url->link('purpletree_multivendor/managesubscriptionplan', 'token=' . $this->session->data['token'], true));
			}
			$this->load->language('purpletree_multivendor/managesubscriptionplan');
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
				...
				if (isset($this->request->post['new_plan_id']) && $this->request->post['new_plan_id'] != '') {
					...
					$invoice_status=$this->model_purpletree_multivendor_managesubscriptionplan->getInvoiceStatusfromSeller($seller_id);
					...
					if(isset($invoice_status)){
						if($invoice_status!=2){
							...
							$this->response->redirect($this->url->link('purpletree_multivendor/managesubscriptionplan/add', 'token=' . $this->session->data['token'].'&seller_id='.$this->request->get['seller_id'] , true));
						}
					}
					$total_seller_assign_product = $this->model_purpletree_multivendor_managesubscriptionplan->getTotalSellerPorduct($seller_id);
					$getnewplanProducts = $this->model_purpletree_multivendor_managesubscriptionplan->getnewplanProducts($plan_id);
					...
					$this->response->redirect($this->url->link('purpletree_multivendor/managesubscriptionplan/plan_confirm', 'token=' . $this->session->data['token'] .'&seller_id='. $seller_id .'&plan_id='.$plan_id.'&start_date='.$start_date.'&status='.$status.'&existing='.$existing.'&action='.$this->request->get['action'], true));
					} else {
		...
		public function edit() {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
				if (isset($this->request->post['new_plan_id']) && $this->request->post['new_plan_id'] != '') {
					...
					$this->session->data['plan_id'] 	= $plan_id;
					$this->session->data['start_date']  = $start_date;
					$this->response->redirect($this->url->link('purpletree_multivendor/managesubscriptionplan/plan_confirm', 'token=' . $this->session->data['token'] .'&seller_id='. $seller_id .'&plan_id='.$plan_id.'&start_date='.$start_date.'&status='.$status.'&existing='.$existing, true));
					} else {
					$this->error['error_select_plan']= $this->language->get('error_please_select_seller');
				}
			} 
		...
		public function active() {
			...
			$this->load->model('purpletree_multivendor/managesubscriptionplan');
			if($id && $seller_id){
				$this->model_purpletree_multivendor_managesubscriptionplan->active($id);
			}
			$this->load->language('purpletree_multivendor/managesubscriptionplan');
			$this->session->data['success'] = $this->language->get('text_plan_activated');
			$this->response->redirect($this->url->link('purpletree_multivendor/managesubscriptionplan/viewallplan', 'token=' . $this->session->data['token'].'&seller_id='.$seller_id, true));
		...
		public function addNewPlan() {
			...
			if($this->config->get('purpletree_multivendor_subscription_plans')!=1){
				$this->response->redirect($this->url->link('purpletree_multivendor/managesubscriptionplan/view', 'token=' . $this->session->data['token'], true));
			} 
			...
			$current_plan=$this->model_purpletree_multivendor_managesubscriptionplan->getPlanBySeller($seller_id);
			$result=$this->model_purpletree_multivendor_managesubscriptionplan->getSubscribePlan($plan_id);
			if($startt_when == 1) {
				
				$current_plan_start_date=$this->model_purpletree_multivendor_managesubscriptionplan->getCurrentPlanByPlanId($seller_id,$plan_id);
			...
			$data['end_date']='';
			$old_invoice_id=$this->model_purpletree_multivendor_managesubscriptionplan->getInvoiceId($seller_id);			
			
			$currentplan=$this->model_purpletree_multivendor_managesubscriptionplan->getCurrentPlan($seller_id);
			
			if($this->config->get("purpletree_multivendor_tax_name")){
				$tax_name=$this->config->get("purpletree_multivendor_tax_name");		
				} else {
				$tax_name='';	
			}
			if($this->config->get("purpletree_multivendor_tax_value")){
				$tax=$this->config->get("purpletree_multivendor_tax_value");		
				} else {
				$tax=0;	
			}
			
			$current_invoice=$this->model_purpletree_multivendor_managesubscriptionplan->getSellerCurrentPlan($seller_id);			
			...
			if($this->config->get("purpletree_multivendor_joining_fees")){
				$joining_fee=$result['joining_fee'];
				} else {
				$joining_fee=0;	
			}
			
			if($this->config->get("purpletree_multivendor_subscription_price")){
			...
			$customer = $this->model_purpletree_multivendor_managesubscriptionplan->getCustomer($seller_id);
			$message='';
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') /* && $this->validateForm() */) {
				/* 	if($this->config->get('purpletree_multivendor_multiple_subscription_plan_active')){ */
				$invoice_id=$this->model_purpletree_multivendor_managesubscriptionplan->addMultipleSellerPlan($data);
				/* } else {
					$invoice_id=$this->model_purpletree_multivendor_managesubscriptionplan->addSellerPlan($data);	
				} */
				$sellerExist=$this->model_purpletree_multivendor_managesubscriptionplan->SellerExist($seller_id);
				$email_subject= $this->language->get('email_subject');
				if(!$sellerExist){
					$sellerExist=$this->model_purpletree_multivendor_managesubscriptionplan->addFirstSellerPlan($seller_id);	
					$email_subject = $this->language->get('email_first_subject');
				}
				
				//if plan free or grand total less then zero
				if($total<=0){
					$this->model_purpletree_multivendor_managesubscriptionplan->enableSellerSubscription($seller_id);
				}
				...
				$mail_subject_vars=array(
						...
						$emailTemplate=$this->model_purpletree_multivendor_mails->getEmailTemplate('admin_to_seller_subscription_plan_mail_by_admin');
						...
						$this->model_purpletree_multivendor_mails->sendMail($customer['email'],$subject,$message);
				//end mail
		...
		public function plan_confirm() {
			...
			$this->load->model('purpletree_multivendor/managesubscriptionplan');
			$invoice_status=$this->model_purpletree_multivendor_managesubscriptionplan->getInvoiceStatuss($seller_id);
			///////////////////
			...
			if(isset($invoice_status)){
				if($invoice_status!=2){
					...
					$this->response->redirect($this->url->link('purpletree_multivendor/managesubscriptionplan/view', '', true));
				}
			} 			
			...
			'href' => $this->url->link('purpletree_multivendor/managesubscriptionplan', 'token=' . $this->session->data['token'] , true)
			);
			...
			$data['action'] = $this->url->link('purpletree_multivendor/managesubscriptionplan/addNewPlan', 'token=' . $this->session->data['token'] . '&seller_id=' . $this->request->get['seller_id'] . $url, true);
			...
			$this->response->setOutput($this->load->view('purpletree_multivendor/plan_confirm', $data));
		}
		...
		public function viewAllPlan() {
			...
			$this->load->language('purpletree_multivendor/managesubscriptionplan');
			$this->load->model('purpletree_multivendor/managesubscriptionplan');
			$subscribed = $this->model_purpletree_multivendor_managesubscriptionplan->getSubscriptionplan($this->request->get['seller_id']);
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST')  /* && $this->validateForm() */ ) {
				if(isset($this->request->post['subscription_status'])) {			
					$this->model_purpletree_multivendor_managesubscriptionplan->changeSubscription($this->request->post['subscription_status'],$seller_id);
					if(isset($this->request->post['subscription_disable']))
					{
						$this->model_purpletree_multivendor_managesubscriptionplan->changeStatus($this->request->post['subscription_disable'],$seller_id);		
					}
			...
			$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('purpletree_multivendor/managesubscriptionplan', 'token=' . $this->session->data['token'] . $url, true)
			);
			...
			$data['add'] = $this->url->link('purpletree_multivendor/managesubscriptionplan/add', 'token=' . $this->session->data['token'] .'&seller_id='. $seller_id .'&action=add', true);
			...
			$total_featured_product = $this->model_purpletree_multivendor_managesubscriptionplan->sellerTotalFeaturedProduct($seller_id);
			
			$total_category_featured_product = $this->model_purpletree_multivendor_managesubscriptionplan->sellerTotalCategpryFeaturedProduct($seller_id);
			...
			$used_products=0;
			if($this->model_purpletree_multivendor_managesubscriptionplan->getsellerProducts($seller_id)){
				$used_products = $this->model_purpletree_multivendor_managesubscriptionplan->getsellerProducts($seller_id);
			}
			...
			if($this->config->get('purpletree_multivendor_multiple_subscription_plan_active')){
				$current_plan=array();
				$current_plan = $this->model_purpletree_multivendor_managesubscriptionplan->getCurrentMultiplePlan($seller_id);
				$current_plan_detail = $this->model_purpletree_multivendor_managesubscriptionplan->getCurrentSellerMultiplePlan($seller_id);
				}  else {
				$current_plan = $this->model_purpletree_multivendor_managesubscriptionplan->getCurrentPlan1($seller_id);
				$current_plan_detail = $this->model_purpletree_multivendor_managesubscriptionplan->getCurrentSellerPlan($seller_id);
			...
			if(!empty($current_plan)){
				foreach($current_plan as $current_key=>$current_value){
					...
					$invoice_status = $this->model_purpletree_multivendor_managesubscriptionplan->invoice_status($current_value['seller_id'],$current_value['plan_id']);
					...
					'validity'=>$current_value['validity'],
					'view_plan'  => $this->url->link('purpletree_multivendor/managesubscriptionplan/view', 'token=' . $this->session->data['token'] .'&seller_id='. $current_value['seller_id'].'&plan_id='.$current_value['plan_id'], true),
					'edit_plan'  => $this->url->link('purpletree_multivendor/managesubscriptionplan/edit', 'token=' . $this->session->data['token'] .'&seller_id='. $current_value['seller_id'].'&plan_id='.$current_value['plan_id'].'&action=edit&status='.$status, true),
					'active_plan'  => $this->url->link('purpletree_multivendor/managesubscriptionplan/active', 'token=' . $this->session->data['token'] .'&id='. $current_value['id'].'&seller_id='. $current_value['seller_id'], true),
			...
			$data['subscription_status'] = $this->model_purpletree_multivendor_managesubscriptionplan->getSubscriptionStatus($seller_id);
			$filter_data = array(
			'seller_id'  => $seller_id,
			...
			if($this->config->get('purpletree_multivendor_multiple_subscription_plan_active')){
				$results = $this->model_purpletree_multivendor_managesubscriptionplan->getSellerMultiplePlansList($filter_data);
				} else {
				$results = $this->model_purpletree_multivendor_managesubscriptionplan->getSellerPlansList($filter_data); 
			}
			
			$data['subscriptions'] = array();
			if(!empty($results)){
				foreach ($results as $result) { 
					$invoice_status = $this->model_purpletree_multivendor_managesubscriptionplan->getInvoiceStatus($result['invoice_id'] );
					...
					*/
					...
					'add_history'  => $this->url->link('purpletree_multivendor/SubscriptionplanInvoice/add', 'token=' . $this->session->data['token'] .'&invoice_id='. $result['invoice_id'] .'', true),
					'view_invoice'  => $this->url->link('purpletree_multivendor/SubscriptionplanInvoice/view', 'token=' . $this->session->data['token'] .'&invoice_id='. $result['invoice_id'] .'', true)
					);	
				}  
			}
			...
			$pagination->url = $this->url->link('purpletree_multivendor/managesubscriptionplan/view', 'token=' . $this->session->data['token'] .'&seller_id=' . $this->request->get['seller_id']. $url . '&page={page}', true);
			...
			$this->response->setOutput($this->load->view('purpletree_multivendor/subscriptionplan_all', $data));
		}
		...
		public function view() {
			...
			$data['add'] = $this->url->link('purpletree_multivendor/managesubscriptionplan/add', 'token=' . $this->session->data['token'] .'&seller_id='. $seller_id .'', true);
			...
			$data['current_plan'] = array();
			//$current_plan=array();
			$current_plan = $this->model_purpletree_multivendor_managesubscriptionplan->getCurrentPlan($seller_id);
			/* if($this->config->get('purpletree_multivendor_multiple_subscription_plan_active')){
				$current_plan=array();
				$current_plan = $this->model_purpletree_multivendor_managesubscriptionplan->getCurrentMultiplePlan($seller_id);
				} 
			...
			/* 		if($current_plan['featured_store']){
				...
				if(!empty($current_plan)) {
				$current_plan['used_products'] = $this->model_purpletree_multivendor_managesubscriptionplan->getsellerProducts($seller_id);
			...
			$results=array();
			$results = $this->model_purpletree_multivendor_managesubscriptionplan->getSellerInvoiceList($filter_data);
			$resultsTotal = $this->model_purpletree_multivendor_managesubscriptionplan->getTotalSellerPlansList($filter_data);
			if($this->config->get('purpletree_multivendor_multiple_subscription_plan_active')){
				$results=array();
				$results = $this->model_purpletree_multivendor_managesubscriptionplan->getSellerInvoiceList2($filter_data);
				$resultsTotal = $this->model_purpletree_multivendor_managesubscriptionplan->getSellerInvoiceList2pagin($filter_data);
			}
			...
			$pagination->page = $page;
			$pagination->limit = $this->config->get('config_limit_admin');
			$pagination->url = $this->url->link('purpletree_multivendor/managesubscriptionplan/view', 'token=' . $this->session->data['token'] .'&seller_id=' . $this->request->get['seller_id'].'&plan_id=' . $this->request->get['plan_id']. $url . '&page={page}', true);
			...
			$this->response->setOutput($this->load->view('purpletree_multivendor/subsacriptionplan_view', $data));
		}
		...
		$this->document->addStyle('view/javascript/purpletreecss/commonstylesheet.css');
			...
			$data['add'] = $this->url->link('purpletree_multivendor/managesubscriptionplan/add', 'token=' . $this->session->data['token'] . $url, true);
			...
			$resultsTotal = $this->model_purpletree_multivendor_managesubscriptionplan->getManageSubscriptionPlanTotal($filter_data);
			$results = $this->model_purpletree_multivendor_managesubscriptionplan->getManageSubscriptionPlan($filter_data);
			//echo"<pre>";print_r($results);die;
			if(!empty($results)){
				foreach ($results as $result) {
					$subscribed = $this->model_purpletree_multivendor_managesubscriptionplan->getSubscriptionplan($result['seller_id']);
					$customerStauts = $this->model_purpletree_multivendor_managesubscriptionplan->getCustomerStatus($result['seller_id']);
					if($this->config->get('purpletree_multivendor_multiple_subscription_plan_active')){
						$no_of_active_plan = $this->model_purpletree_multivendor_managesubscriptionplan->getNoOfActiveMultiplePlan($result['seller_id']);
						if(!$no_of_active_plan){
							$no_of_active_plan=0;	
						}
						} else {
						$no_of_active_plan = $this->model_purpletree_multivendor_managesubscriptionplan->getNoOfActivePlan($result['seller_id']);
						if(!$no_of_active_plan){
							$no_of_active_plan=0;	
						}
					}
					...
					'view'        		=> $this->url->link('purpletree_multivendor/managesubscriptionplan/viewallplan', 'token=' . $this->session->data['token'] . '&seller_id=' . $result['seller_id'] . $url, true)
					);
				} 
			}
			...
			$data['sort_name'] = $this->url->link('purpletree_multivendor/managesubscriptionplan', 'token=' . $this->session->data['token'] . '&sort=name' . $url, true);
			$data['sort_sort_order'] = $this->url->link('purpletree_multivendor/managesubscriptionplan', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, true);
			...
			$pagination->page = $page;
			$pagination->limit = $this->config->get('config_limit_admin');
			$pagination->url = $this->url->link('purpletree_multivendor/managesubscriptionplan', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
			...
			$this->response->setOutput($this->load->view('purpletree_multivendor/manage_subscription_plan_list', $data));
		}
		
		protected function getForm() {
			
			if (!isset($this->request->get['seller_id']) || $this->request->get['seller_id'] =='') {
				$this->response->redirect($this->url->link('purpletree_multivendor/managesubscriptionplan/viewallplan', 'token=' . $this->session->data['token'], true));
			}
			...
			$current_plan = $this->model_purpletree_multivendor_managesubscriptionplan->getPlanById($seller_id,$plan_id) ;
			
			$active_plan = $this->model_purpletree_multivendor_managesubscriptionplan->getActivePlan($seller_id,$plan_id) ;
			
			if($this->config->get('purpletree_multivendor_multiple_subscription_plan_active')){
				$active_plan = $this->model_purpletree_multivendor_managesubscriptionplan->getMultipleActivePlan($seller_id,$plan_id) ;
			}
			...
			$data['seller_name'] = $this->model_purpletree_multivendor_managesubscriptionplan->getSellerName($seller_id);
			//die;
			...
			$data['cancel'] = $this->url->link('purpletree_multivendor/managesubscriptionplan/view','token=' . $this->session->data['token'] . '&seller_id=' . $this->request->get['seller_id'] . $url, true);
			...
			$this->response->setOutput($this->load->view('purpletree_multivendor/manage_subscription_plan_form', $data));
		}
		...
		public function autocomplete() {
			...
			if (isset($this->request->get['filter_store_name'])) {
				...
				$results = $this->model_purpletree_multivendor_managesubscriptionplan->getStoreName($filter_data);
			...
			if (isset($this->request->get['filter_plan_name'])) {
				...
				$results = $this->model_purpletree_multivendor_managesubscriptionplan->getPlanName($filter_data);
				if(!empty($results)){
					foreach ($results as $result) {
						$json[] = array(
						'plan_id' => $result['plan_id'],
