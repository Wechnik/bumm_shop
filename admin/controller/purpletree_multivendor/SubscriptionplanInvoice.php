<?php
class ControllerPurpletreeMultivendorSubscriptionplanInvoice extends Controller {
		private $error = array();
		
		public function index() {
			$this->load->language('purpletree_multivendor/subscriptionplanInvoice');
			$this->document->setTitle('Offline Payment');
			$this->load->model('purpletree_multivendor/subscriptionplanInvoice');
		}
		...
		public function view() {
		   $this->document->addStyle('view/javascript/purpletreecss/commonstylesheet.css');
			...
			$seller_id=$this->model_purpletree_multivendor_subscriptionplanInvoice->getsellerfromInvoice($invoice_id);
			if(!$seller_id) {
				$this->response->redirect($this->url->link('purpletree_multivendor/managesubscriptionplan', 'token=' . $this->session->data['token'] , true));
			}
			$seller_store=$this->model_purpletree_multivendor_subscriptionplanInvoice->getStoreDetail($seller_id);
			$this->load->model('purpletree_multivendor/stores');
			$cus_seller_email = $this->model_purpletree_multivendor_stores->getCustomerEmailId($seller_id);		
			...
			$data['payment_history']=$this->model_purpletree_multivendor_subscriptionplanInvoice->getInvoiceHistory($invoice_id);
			$invoceddetail = $this->model_purpletree_multivendor_subscriptionplanInvoice->getinvoice($invoice_id);
			$data['created_date'] = '';
			$data['current_status'] ='';
			if($invoceddetail) {
				$data['created_date']= date('d-m-Y', strtotime($invoceddetail['created_date']));
				$data['current_status']=$this->model_purpletree_multivendor_subscriptionplanInvoice->getstausfromid($invoceddetail['status_id']);
			}
			$old_invoice_id=$this->model_purpletree_multivendor_subscriptionplanInvoice->getInvoiceId($seller_id);
			//$old_invoice_id=$this->request->get['old_invoice_id'];
			$data['invoice_id']=$invoice_id;
			
			if(isset($invoice_id)){
				$data['invoice_data']=$this->model_purpletree_multivendor_subscriptionplanInvoice->getPlanId($invoice_id,$old_invoice_id);
			}
			///////////////////
			...
			$planId = $this->model_purpletree_multivendor_subscriptionplanInvoice->getsellerplanid($invoice_id);
			$data['newplan'] = array();
			if(isset($data['invoice_data']['invoice']['plan_id'])){
				$result=$this->model_purpletree_multivendor_subscriptionplanInvoice->getSubscribePlan($data['invoice_data']['invoice']['plan_id']);
			}
			...
			if(!empty($result)){
				...
				'end_date'        => ($result['end_date']!='0000-00-00 00:00:00')?date('d/m/Y',strtotime($result['end_date'])):date('d/m/Y', strtotime($result['start_date']. ' + '.$result['validity'].' days'))
				);	
			}
			...
			if(!empty($data['payment_history'])) {
				foreach($data['payment_history'] as $key => $paymentss) {
					$data['payment_history'][$key]['status'] = $this->model_purpletree_multivendor_subscriptionplanInvoice->getstausfromid($paymentss['status_id']);
				}
			}
			...
			$this->response->setOutput($this->load->view('purpletree_multivendor/subscription_invoice', $data));
		}
		...
		public function add() {
			...
			$invoice_id = $this->request->get['invoice_id'];
			$currentStatus = $this->model_purpletree_multivendor_subscriptionplanInvoice->getinvoice($invoice_id);
			$data['currentstatus'] = '1';
			if($currentStatus) {
				$data['currentstatus'] = $currentStatus;
			}
			if ($this->request->server['REQUEST_METHOD'] == 'POST') {
				$this->model_purpletree_multivendor_subscriptionplanInvoice->addOfflinePayment($this->request->post,$invoice_id);
				$this->session->data['success'] = "Payment History Added successully";
				$this->response->redirect($this->url->link('purpletree_multivendor/managesubscriptionplan', 'token=' . $this->session->data['token'], true));
			}
			$data['invoice_id'] = $this->request->get['invoice_id'];
			$data['statuslist'] = $this->model_purpletree_multivendor_subscriptionplanInvoice->getstatuslist();
			...
			$data['saveAndEnableSubscription']=$this->url->link('purpletree_multivendor/SubscriptionplanInvoice/saveAndEnableSubscription', 'token=' . $this->session->data['token'].'&invoice_id='.$invoice_id, true);
			$data['text_payment_mode'] = $this->language->get('text_payment_mode');
			$data['text_status'] = $this->language->get('text_status');
			$data['text_transaction_id'] = $this->language->get('text_transaction_id');
			$data['text_comment'] = $this->language->get('text_comment');
			...
			$data['button_saveAndEnableSubscription']=$this->language->get('button_saveAndEnableSubscription'); 
			$data['text_offline_payment_form_invoice_id']=$this->language->get('text_offline_payment_form_invoice_id'); 
			$data['header'] = $this->load->controller('common/header');
			$data['column_left'] = $this->load->controller('common/column_left');
			$data['footer'] = $this->load->controller('common/footer');
			$this->response->setOutput($this->load->view('purpletree_multivendor/offline_payment_form', $data));
		}
		public function saveAndEnableSubscription() {
			
			if (!isset($this->request->get['invoice_id']) || $this->request->get['invoice_id'] == '') {
				$this->response->redirect($this->url->link('purpletree_multivendor/managesubscriptionplan', 'user_token=' . $this->session->data['user_token'], true));
			}
			$this->load->language('purpletree_multivendor/subscriptionplanInvoice');
			$this->load->model('purpletree_multivendor/subscriptionplanInvoice');
			$invoice_id = $this->request->get['invoice_id'];
			if ($this->request->server['REQUEST_METHOD'] == 'POST') {
				...
				$this->model_purpletree_multivendor_subscriptionplanInvoice->enableSubscription($invoice_id);
				$this->session->data['success'] = "Payment History Added successully";
				$this->response->redirect($this->url->link('purpletree_multivendor/managesubscriptionplan', 'token=' . $this->session->data['token'], true));
			}
		}
