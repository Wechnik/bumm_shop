<?php
class ControllerPurpletreeMultivendorSellerenquiries extends Controller {
		private $error = array();
		...
		public function index() {
			...
			$this->load->model('purpletree_multivendor/sellerenquiries');
		...
		public function delete() {
			...
			if (isset($this->request->post['selected']) ) {
				foreach ($this->request->post['selected'] as $message_id) {
					$this->model_purpletree_multivendor_sellerenquiries->deleteMessage($message_id);
				}
				...
				$this->response->redirect($this->url->link('purpletree_multivendor/sellerenquiries', 'token=' . $this->session->data['token'] . $url, true));
			}
		...
		protected function getList() {
			...
			'href' => $this->url->link('purpletree_multivendor/sellerenquiries', 'token=' . $this->session->data['token'] . $url, true)
			);
			
			$data['send_all_message'] = $this->url->link('purpletree_multivendor/sellerenquiries/sendsellermessage', 'token=' . $this->session->data['token'] . '&sendtoall=1', true);
			
			$data['view'] = $this->url->link('purpletree_multivendor/sellerenquiries/view', 'token=' . $this->session->data['token'] . $url, true);
			$data['delete'] = $this->url->link('purpletree_multivendor/sellerenquiries/delete', 'token=' . $this->session->data['token'] . $url, true);
			...
			$contact_total = $this->model_purpletree_multivendor_sellerenquiries->getTotalsellerenquiries($filter_data);
			
			$results = $this->model_purpletree_multivendor_sellerenquiries->getsellerenquiries($filter_data);
			if(!empty($results)){
				foreach ($results as $result) {
					...
					if (!$this->customer->validateSeller()) {
						...
						$view = $this->url->link('purpletree_multivendor/sellerenquiries', 'token=' . $this->session->data['token'], true);
						} else {
						$view = $this->url->link('purpletree_multivendor/sellerenquiries/sendsellermessage', 'token=' . $this->session->data['token'] . '&id=' . $result['id'].'&seller_id='.$result['seller_id'] . $url, true);
					}
			...
			$pagination->url = $this->url->link('purpletree_multivendor/sellerenquiries', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
			...
			$this->response->setOutput($this->load->view('purpletree_multivendor/sellerenquiries_list', $data));
		}
		public function sendSellerMessage() {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
				if($this->request->post['seller_id']){
					$this->model_purpletree_multivendor_sellerenquiries->addSellerMessage($this->request->post);						
					$this->sendMailtoSeller();			
					$this->session->data['success'] = $this->language->get('text_success_seller');
					$this->response->redirect($this->url->link('purpletree_multivendor/sellerenquiries/sendsellermessage', 'token=' . $this->session->data['token'].'&seller_id='.$this->request->post['seller_id'] . $url, true));
					} elseif($this->request->post['sendtoall']) {
					$allmails = $this->model_purpletree_multivendor_sellerenquiries->getSellerAllMails();
					if(!empty($allmails)){
						foreach($allmails as $contact) {
				$this->load->model('purpletree_multivendor/mails');									
					...
					$mail_msg_vars=array(
						...
						$emailTemplate=$this->model_purpletree_multivendor_mails->getEmailTemplate('admin_seller_contact_by_admin');
						...
						$this->model_purpletree_multivendor_mails->sendMail($contact,$subject,$message);
						}
					}
					$this->model_purpletree_multivendor_sellerenquiries->sendAllSellerMessage($this->request->post['message']);
			...
			$seller_messages=$this->model_purpletree_multivendor_sellerenquiries->getSellerMessage($filter_data);
			...
			$contact_total=$this->model_purpletree_multivendor_sellerenquiries->getSellerTotalMessage($filter_data);
			
			$pagination = new Pagination();
			$pagination->total = $contact_total;
			$pagination->page = $page;
			$pagination->limit = $this->config->get('config_limit_admin');
			$pagination->url = $this->url->link('purpletree_multivendor/sellerenquiries/sendsellermessage', 'token=' . $this->session->data['token'].'&seller_id='.$seller_id . $url . '&page={page}', true);
			
			$data['pagination'] = $pagination->render();
			
			$data['results'] = sprintf($this->language->get('text_pagination'), ($contact_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($contact_total - $this->config->get('config_limit_admin'))) ? $contact_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $contact_total, ceil($contact_total / $this->config->get('config_limit_admin')));
			
			$data['action'] = $this->url->link('purpletree_multivendor/sellerenquiries/sendMessage', 'token=' . $this->session->data['token'] . '&seller_id='. $data['seller_id'], true);
			$data['cancel'] = $this->url->link('purpletree_multivendor/sellerenquiries', 'token=' . $this->session->data['token'] , true);
			
			$data['header'] = $this->load->controller('common/header');
			$data['column_left'] = $this->load->controller('common/column_left');
			$data['footer'] = $this->load->controller('common/footer');
			
			$this->response->setOutput($this->load->view('purpletree_multivendor/sellerenquiries_form', $data));
		...
		public function view() {
			...
			$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('purpletree_multivendor/sellerenquiries', 'token=' . $this->session->data['token'] , true)
			);
			
			$data['action'] = $this->url->link('purpletree_multivendor/sellerenquiries/view', 'token=' . $this->session->data['token'] . '&id=' . $this->request->get['id'].'&seller_id='. $this->request->get['seller_id'], true);
			...
			if (isset($this->request->get['id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
				$message_info = $this->model_purpletree_multivendor_sellerenquiries->getMessage($this->request->get['id']);
			}
		...
		public function sendMailtoSeller() {
			$seller_info = $this->model_purpletree_multivendor_sellerenquiries->getSellerDetail($this->request->get['seller_id']);		
			if(!empty($seller_info)){						
				...
				$mail_msg_vars=array(
						...
						$emailTemplate=$this->model_purpletree_multivendor_mails->getEmailTemplate('admin_seller_contact_mail_by_admin');
						$subject=$this->model_purpletree_multivendor_mails->replaceVariables($mail_subject_vars,$emailTemplate['new_subject']);
						$message=$this->model_purpletree_multivendor_mails->replaceVariables($mail_msg_vars,$emailTemplate['new_message']);
						$this->model_purpletree_multivendor_mails->sendMail($seller_email,$subject,$message);	
			}	
		}
}
?>