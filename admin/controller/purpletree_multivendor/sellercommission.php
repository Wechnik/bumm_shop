<?php
class ControllerPurpletreeMultivendorSellercommission extends Controller{
		private $error = array();
		...
		$this->document->addStyle('view/javascript/purpletreecss/commonstylesheet.css');
			...
			'href' => $this->url->link('purpletree_multivendor/sellercommission', 'token=' . $this->session->data['token'] . $url, true)
			);
			...
			$data['seller_commission'] = $this->language->get('seller_commission');
			...
			$seller_commissions1 = $this->model_purpletree_multivendor_sellercommission->getCommissions($filter_data);
			$seller_commissions111 = $this->model_purpletree_multivendor_sellercommission->getCommissions111($filter_data);
			...
			if($seller_commissions){
				foreach($seller_commissions as $seller_commission){
					//$seller_commissions2 = $this->model_purpletree_multivendor_sellercommission->getCommissions11($seller_commission1['order_id']);
					//foreach($seller_commissions2 as $seller_commission){
						...
						'store_url' => $this->url->link('purpletree_multivendor/stores/edit&store_id='.$seller_commission['store_id'], 'token=' . $this->session->data['token'], true),
			...
			$pagination->url = $this->url->link('purpletree_multivendor/sellercommission', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
			...
			$data['generated_invoice_order_id'] = $this->session->data['generated_invoice_order_id'];
			 ...
			 $data['generated_invoice_order_id'] = '';
			}
			$data['seller_stores'] = $this->model_purpletree_multivendor_sellercommission->getSellerstore();
			...
			$data['commission_invoice'] = $this->url->link('purpletree_multivendor/sellercommission/generate', 'token=' . $this->session->data['token'] . $url, true);
			$this->response->setOutput($this->load->view('purpletree_multivendor/commission_list', $data));
		}
		public function generate(){
			$this->load->language('purpletree_multivendor/ptsmultivendor');
			if (!$this->customer->validateSeller()) {
				$this->session->data['error_warning'] = $this->language->get('error_license');
				$this->response->redirect($this->url->link('purpletree_multivendor/sellercommission', 'token=' . $this->session->data['token'], true));
			}
			$this->load->language('purpletree_multivendor/sellercommission');
			...
			foreach($this->request->post['selected'] as $key => $value){
			 $commission_invoice_check = $this->model_purpletree_multivendor_sellercommission->checkCommissionInvoice($value,$this->request->get['seller_id']);
			...
			if (isset($this->request->post['selected'])) {
				...
				try {
					...
					if(!empty($commisionss)) {
						foreach ($commisionss as $commisionid => $order_id) {
							$commisionssss = $this->model_purpletree_multivendor_sellercommission->getCommissionsforinvoide($commisionid);
						...
						foreach($uniqueSoId as $vvvv){
							$total_price[$vvvv['seller_id']][]= $this->model_purpletree_multivendor_commissioninvoice->getCommissionTotal($vvvv['order_id'],$vvvv['seller_id']);
						}
						...
						if(!empty($t_comm)){
							foreach($t_comm as $seller_idd=>$seller_commm){
								...
								$linkid[$seller_idd] = $this->model_purpletree_multivendor_commissioninvoice->savelinkid($total_price,$seller_commm,$total_pay_amount);
							}
						}
						
						
						foreach ($commisionss as $commisionid => $order_id) {
							$commisionsss = $this->model_purpletree_multivendor_sellercommission->getCommissionsforinvoide($commisionid);
							...
							if(!empty($commisionsss)) {
								if($commisionsss['order_id'] == $order_id && $commisionsss['invoice_status'] == 0) {
									...
									$this->model_purpletree_multivendor_commissioninvoice->saveCommisionInvoice($commisionsss,$linkidd);
				...
				if(!empty($linkid)){
					$this->response->redirect($this->url->link('purpletree_multivendor/commissioninvoice/commissionInvoice', 'token=' . $this->session->data['token'], true));
				}
			}
			}else{
			...
			$this->response->redirect($this->url->link('purpletree_multivendor/sellercommission', 'token=' . $this->session->data['token'].$seller_id , true));
