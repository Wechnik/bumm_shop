<?php
class ControllerPurpletreeMultivendorCategoriescommission extends Controller{
		private $error = array();
		public function index(){		
			...
			$this->load->model('purpletree_multivendor/categoriescommission');
		...
		protected function getList() {
			...
			'href' => $this->url->link('purpletree_multivendor/categoriescommission', 'token=' . $this->session->data['token'] . $url, true)
			);
			
			if (!isset($this->request->get['category_id'])) {
				$data['action'] = $this->url->link('purpletree_multivendor/categoriescommission/add', 'token=' . $this->session->data['token'] . $url, true);
			} 
			
			if (!$this->customer->validateSeller()) {
				$this->load->language('purpletree_multivendor/ptsmultivendor');
				$this->session->data['error_warning'] = $this->language->get('error_license');
				$data['action'] = $this->url->link('purpletree_multivendor/categoriescommission', 'token=' . $this->session->data['token'] . $url, true);
				} else {
				if (!isset($this->request->get['category_id'])) {
					$data['action'] = $this->url->link('purpletree_multivendor/categoriescommission/add', 'token=' . $this->session->data['token'] . $url, true);
				}				
			...
			$commission_total = $this->model_purpletree_multivendor_categoriescommission->getTotalCategoriesCommission($filter_data);
			...
			$data['text_add_commission'] = $this->language->get('text_add_commission');
			$data['entry_categories'] = $this->language->get('entry_categories');
			$data['text_Available_commission'] = $this->language->get('text_Available_commission');
			...
			$data['filter_commission_fixed'] = $this->language->get('filter_commission_fixed');			
			...
			$data['button_add1'] = $this->language->get('button_add1');
			...
			$data['sort_commission'] = $this->url->link('purpletree_multivendor/categoriescommission', 'token=' . $this->session->data['token'] . '&sort=commission' . $url, true);			
			$url = '';
			...
			$pagination->url = $this->url->link('purpletree_multivendor/categoriescommission', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
			...
			$data['show_seller_group'] = $this->config->get('purpletree_multivendor_seller_group');
			
			$categoriescommissions = $this->model_purpletree_multivendor_categoriescommission->getCategoriesCommission($filter_data,$data['show_seller_group']);
			//echo "<pre>"; print_r($categoriescommissions); die; 
			...
			foreach($categoriescommissions as $categoriescommission)
			{
				...
				'editaction' => $this->url->link('purpletree_multivendor/categoriescommission/edit', 'token=' . $this->session->data['token'] . '&commission_id=' . $categoriescommission['id'] . $url, true),
				'deleteaction' => $this->url->link('purpletree_multivendor/categoriescommission/delete', 'token=' . $this->session->data['token'] . '&commission_id=' . $categoriescommission['id'] . $url, true)				
			...
			$this->response->setOutput($this->load->view('purpletree_multivendor/categories_commission', $data));
		...
		public function add(){
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->request->post['filter_id'] && $this->request->post['filter_commission'] && is_numeric($this->request->post['filter_commission']) && (is_numeric($this->request->post['filter_commission_fixed']) || ($this->request->post['filter_commission_fixed'] == ''))) { 			
				...
				$this->model_purpletree_multivendor_categoriescommission->addCategoriesCommission($this->request->post);
			...
			$url = "";
			$this->response->redirect($this->url->link('purpletree_multivendor/categoriescommission', 'token=' . $this->session->data['token'] . $url, true));			
		}
		...
		public function edit(){	
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
				if(is_numeric($this->request->post['filter_commission']) && (is_numeric($this->request->post['filter_commission_fixed']) || ($this->request->post['filter_commission_fixed'] == ''))) {
					...
					$this->model_purpletree_multivendor_categoriescommission->editCategoriesCommission($this->request->post);
            ...
            $commission = $this->model_purpletree_multivendor_categoriescommission->getCommission($this->request->get['commission_id']);
			$data['category_id'] = $commission['category_id'];
			
			$categoryname = $this->model_purpletree_multivendor_categoriescommission->getCategoryName($data['category_id']);
			
			$data['selected_seller_group'] = $commission['seller_group'];	
		    ...
		    $data['column_left'] = $this->load->controller('common/column_left');
		    $data['footer'] = $this->load->controller('common/footer');	
			$this->response->setOutput($this->load->view('purpletree_multivendor/categories_commissionedit',$data));
		}
		public function delete() {
			...
			if($this->request->get['commission_id']){
				$this->model_purpletree_multivendor_categoriescommission->deleteCategoriesCommission($this->request->get['commission_id']);
		...
		/* 	public function autocomplete() {
			...
			$getAllCategories = $this->model_purpletree_multivendor_categoriescommission->getAllCategories();
			$results = $this->model_purpletree_multivendor_categoriescommission->getCategories($filter_data);
			$all_categ = array();
			foreach($getAllCategories as $catt) {
			$all_categ[] = $catt['category_id'];
			}	
			foreach ($results as $result) {
			if(!in_array($result['category_id'],$all_categ)) {
			$json[] = array(
		...
		public function autocomplete() {
			...
			if (isset($this->request->get['filter_name'])) {
				...
				if($this->config->get('purpletree_multivendor_allow_category')){
					$allowedCategories = $this->config->get('purpletree_multivendor_allow_category');
				}	
				foreach ($results as $result) {
					if($this->config->get('purpletree_multivendor_allow_categorytype')==0){
						...
						} else if($this->config->get('purpletree_multivendor_allow_categorytype')==1) {
						$json[] = array(
						'vendor_id'       => $result['category_id'],
						'name'            => strip_tags(html_entity_decode($result['name'], ENT_QUOTES, 'UTF-8'))
						);
