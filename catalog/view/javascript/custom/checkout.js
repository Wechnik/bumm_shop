...
function checkoutPayment(){
  return new Promise( (resolve, reject) => {
    $.ajax({
      ...
      type: 'post',
      data: $('#custom-payment input[name=\'payment_method\']:checked, #custom-control input[name=\'agree\']:checked, #custom-add select'),
      dataType: 'json',
      beforeSend: function(){
        $('#button-custom-order').button('loading');
      },
    ...
    $.ajax({
      ...
      success: function(html) {
          ...
          setTimeout(function(){
            ...
            $(document).find('#custom-confirm button, #button-confirm').trigger('click');
          }, 100);
          reject('confirm');
      },
      complete: function(){
