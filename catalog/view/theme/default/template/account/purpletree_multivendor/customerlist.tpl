				...
				<?php foreach($sellercontacts as $contact ){ ?>
					<table class="pts-table pts-table-striped pts-table-bordered">
						<tr>
							...
							<td class="pts-text-right"><?php echo $contact['date_added']; ?>
								<?php if($contact['customer_id']!=0 and $config_contactseller==1 ){ ?>
								<a href='<?php echo $contact['reply']; ?>' > Reply </a> <?php } ?> </td>
						</tr>
						<?php foreach($contact['customer_messages'] as $key=>$message){ ?>
							
							<tr>
								<td colspan="2">
									<p>
										<?php if($contact['contact_from'][$key]==1){ ?>
										<b>seller - </b> <?php } else { ?><b>customer - </b> <?php } echo $message; ?> </p>
								</td>
							</tr>
						<?php } ?>
					</table>
