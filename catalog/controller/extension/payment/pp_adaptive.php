...
class ControllerExtensionPaymentPPAdaptive extends Controller {
	...
	//$this->log->write('callback pp adaptive');
		...
		//$this->log->write('callback if order_info');
			...
			if ((strcmp($response, 'VERIFIED') == 0 || strcmp($response, 'UNVERIFIED') == 0) && isset($this->request->post['payment_status'])) {
				...
				$sellersss = $this->model_purpletree_multivendor_sellerorder->getOrderSeller($order_id);
				if(!empty($sellersss)) {
					if ($this->config->get('payment_pp_adaptive_debug')) {
				...
				$this->log->write($sellersss);
					...
					$this->log->write($sellerr_id1['seller_id']);
						...
						$comment = '';
						$this->model_purpletree_multivendor_sellerorder->addOrderHistory($order_id,$sellerr_id1['seller_id'], $order_status_id, $comment,false,false);
							if ($this->config->get('payment_pp_adaptive_debug')) {
					$this->log->write('b');
							}
				...
				$result1 = curl_exec($ch1);
					...
					$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "purpletree_vendor_adaptive_paykey where order_id = '".(int)$order_id."'"); 
						...
						$invoicedata = array();
								...
								//}
								//$this->model_purpletree_multivendor_sellerorder->addOrderHistory($order_id,$valuess['seller_id'], $order_status_id1, $messageseller, false,false);
							}
							$commisionsss=array();
	...
	public function getpayoutitemsdeatils($order_id = "") {
		$data_items = array();
	//if ( $this->config->get('purpletree_multivendor_commission_status')!= null) {
					...
					if(!empty($sellerorders->rows)) {
						foreach($sellerorders->rows as $sellerorder) {
							...
							if($totalshipingorder != 0) {
								if (null !== $this->config->get('purpletree_multivendor_shipping_commission')) {
									if(!array_key_exists($sellerorder['seller_id'],$dsdsds)) {
										$shippingcommision = (($this->config->get('purpletree_multivendor_shipping_commission')*$totalshipingorder)/100);
							...
							$comifixs = 0;
							 $globalcomifixs = 0;
							 if(($this->config->get('purpletree_multivendor_fix_commission')) && ($this->config->get('purpletree_multivendor_fix_commission') > 0)){
							  $globalcomifixs = $this->config->get('purpletree_multivendor_fix_commission');
								 }
							if(!empty($commission_cat)) {
								foreach($commission_cat as $catts) {
									foreach($catts as $catt) {
										...
										if (null !== $this->config->get('purpletree_multivendor_seller_group') && $this->config->get('purpletree_multivendor_seller_group') == 1) {
							...
							elseif(isset($seller_commission['store_commission']) && ($seller_commission['store_commission'] != NULL || $seller_commission['store_commission'] != '')){
								...
								$comipercen = $this->config->get('purpletree_multivendor_commission');
								$commission = (($sellerorder['total_price']*$this->config->get('purpletree_multivendor_commission'))/100)+$shippingcommision+$globalcomifixs;
