...
class ControllerExtensionModulePurpletreeSellerfeatured extends Controller {
		public function index($setting) {
		 ...
		 if ($direction=='rtl'){
			...
			if ($this->config->get('purpletree_sellerfeatured_limit')) {
			$products = array_slice($productss, 0, (int)$this->config->get('purpletree_sellerfeatured_limit'));
			}else{
			$products = array_slice($productss, 0, 5);
			}
			$image_height = 200;
			$image_width = 200;
			if ($this->config->get('purpletree_sellerfeatured_height')) {
			  $image_height = $this->config->get('purpletree_sellerfeatured_height');
			}
			if ($this->config->get('purpletree_sellerfeatured_width')) {
			  $image_width = $this->config->get('purpletree_sellerfeatured_width');
			}
			/* get active skin */
					...
					//if($count < 8) {
						...
						if(!in_array($product['product_id'],$prodssarray)) {
							...
							$data['show_seller_address'] = $this->config->get('purpletree_multivendor_show_seller_address');
								$seller_detail = array();
								$pts_quick_status = '';
								$seller_detail = $this->model_purpletree_multivendor_sellerproduct->getSellername($product_info['product_id']);
