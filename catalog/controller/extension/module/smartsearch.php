...
class ControllerExtensionModuleSmartsearch extends Controller {
	public function index() {
		...
		if (isset($this->request->post['search']) && $this->request->post['search']) {
			...
			$order = 'ASC';
			$page = 1;
			$limit = ($this->config->get('smartsearch_limit') > 0 ? $this->config->get('smartsearch_limit') : 10);
			...
			if ($this->config->get('smartsearch_button_all')) {
			...
			foreach ($products as $keyproduct => $product) {
				...
				$image = '';
				if ($this->config->get('smartsearch_image')) {
					if ($this->config->get('smartsearch_image_width')) {
						$smartsearch_image_width = $this->config->get('smartsearch_image_width');
					} else {
						$smartsearch_image_width = 50;
					}

					if ($this->config->get('smartsearch_image_height')) {
						$smartsearch_image_height = $this->config->get('smartsearch_image_height');
					} else {
