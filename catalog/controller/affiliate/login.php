...
class ControllerAffiliateLogin extends Controller {
	...
	public function index() {
        ...
        $data['entry_email_hold'] = $this->language->get('entry_email_hold');
	...
	protected function validate() {
	  $pos = strpos($this->request->post['email'], '@');
      if ($pos === false) {
      $email = '';
      $telefon = $this->request->post['email'];
      ...
      $login = $this->request->post['email'];
		...
		$login_info = $this->model_affiliate_affiliate->getLoginAttempts($login);
	...
	/*	$affiliate_info = $this->model_affiliate_affiliate->getAffiliateByEmail($this->request->post['email']);
		...
		if (!$this->error) {
			if (!$this->affiliate->login($email, $telefon, $this->request->post['password'])) {
			    if (!$telefon) {
				$this->error['warning'] = $this->language->get('error_login');
			    } else {
                $this->error['warning'] = $this->language->get('error_login_telefon');
			    }
				$this->model_affiliate_affiliate->addLoginAttempt($login);
			} else {
				$this->model_affiliate_affiliate->deleteLoginAttempts($login);
			}
		}
