<?php
class ControllerAccountPurpletreeMultivendorSellercontact extends Controller {
	...
	public function customerContactlist(){
			...
			$data['breadcrumbs'] = array();
				...
				'href' => $this->url->link('account/purpletree_multivendor/sellercontact/customerContactlist', '', true)
				);
					...
					foreach($results1 as $re) {
						...
						if(!empty($results2)) {
							foreach($results2 as $result){
								...
								'reply'			 => $this->url->link('account/purpletree_multivendor/sellercontact/customerreply','id='. $result['id'], true)
								);
							}
						}
					}
				...
				$data['config_contactseller']=$this->config->get('purpletree_multivendor_seller_contact');
				
				} else {
				$this->session->data['redirect'] = $this->url->link('account/purpletree_multivendor/sellercontact/customerContactlist','', true);
			...
			$pagination->url = $this->url->link('account/purpletree_multivendor/sellercontact/customercontactlist', '&page={page}',true);
	...
	public function sellercontactlist(){
			$data['contact_mode'] = $this->config->get('purpletree_multivendor_seller_contact');
			$this->load->model('purpletree_multivendor/sellercontact');
			$this->load->language('purpletree_multivendor/sellercontact');
			...
			$data['breadcrumbs'] = array();
				...
				$data['breadcrumbs'][] = array(
				'text' => $this->language->get('text_heading'),
				'href' => $this->url->link('account/purpletree_multivendor/sellercontact/sellercontactlist', '', true)
				);
				$data['text_heading'] = $this->language->get('text_heading');
				$filter_data = array(
				'start'              => ($page - 1) * $limit,
					...
					foreach($results1 as $re) {
						...
						if(!empty($results2)) {
							foreach($results2 as $result){
								...
								'date_added' 	 => date($this->language->get('date_format_short'), strtotime($result['created_at'])),
								'reply'			 => $this->url->link('account/purpletree_multivendor/sellercontact/reply','id='. $result['id'], true)
								);
				...
				$pagination->url = $this->url->link('account/purpletree_multivendor/sellercontact/sellercontactlist', '&page={page}',true);
	...
	public function reply() {
			...
			if($this->config->get('purpletree_multivendor_seller_contact')==1){
				if (!$this->customer->isLogged()) {
					$this->session->data['redirect'] = $this->url->link('account/purpletree_multivendor/sellercontact/reply', 'id='.$this->request->get['customer_id'], true);
					$this->response->redirect($this->url->link('account/login', '', true));
				}
			}
			$data['allow_seller_to_reply'] = $this->config->get('purpletree_multivendor_allow_seller_to_reply');
			...
			if(!isset($this->request->get['id'])){
				$this->response->redirect($this->url->link('account/purpletree_multivendor/sellercontact/sellercontactlist','',true));	
			}
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validatemessage()) {
				...
				$chat_id=$this->model_purpletree_multivendor_sellercontact->addContact($dataa);
				$attached_files 	= $this->model_purpletree_multivendor_sellercontact->getAttachedEnquiriesFile($chat_id);
						$attach_file=array();
				...
				$this->model_purpletree_multivendor_vendor->ptsSendMail($reciver,$email_subject,$email_message,$attach_file);
			...
			'href' => $this->url->link('account/purpletree_multivendor/sellercontactlist','',true)
			);
			...
			$data['text_attach_file'] = $this->language->get('text_attach_file');
			$data['text_upload_file'] = $this->language->get('text_upload_file');
			$data['text_action'] = $this->language->get('text_action');
			$data['text_files'] = $this->language->get('text_files');			
			$data['button_submit'] = $this->language->get('button_submit');
			
			//$data['reply'] =$this->url->link('account/purpletree_multivendor/sellerorder/seller_order_info', '', true);
			...
			if(!empty($results2)) {
				foreach($results2 as $result){
				$attached_files 	= $this->model_purpletree_multivendor_sellercontact->getAttachedEnquiriesFile($result['id']);
					...
					'customer_messages'       => $result['customer_message'],
					'date_added' => date($this->language->get('date_format_short'), strtotime($result['created_at'])),
					'attached_file'=>$attachedFileLinks 
					);
				}
			}
			$this->model_purpletree_multivendor_sellercontact->updateSeenContact($seller_id,$customerid);
			// Captcha
	...
	public function customerReply() {
			...
			if(isset($this->request->get['seller_id'])){
				$seller_storessst = $this->model_purpletree_multivendor_sellercontact->checkSellerVacations($this->request->get['seller_id']);
			...
			if($this->config->get('purpletree_multivendor_seller_contact')==1){
				if (!$this->customer->isLogged()) {
					if(isset($this->request->get['seller_id'])) {
						$this->session->data['redirect'] = $this->url->link('account/purpletree_multivendor/sellercontact/customerReply', 'seller_id='.$this->request->get['seller_id'], true);
						} elseif(isset($this->request->get['id'])) {
						$this->session->data['redirect'] = $this->url->link('account/purpletree_multivendor/sellercontact/customerReply', 'id='.$this->request->get['id'], true);
					}
					$this->response->redirect($this->url->link('account/login', '', true));
				}
			}
			...
			if (!$this->customer->isLogged()) {
				...
				if(!empty($_FILES["attached_file"]['tmp_name'])){
					...
					unset($this->session->data['ptsmv_current_page_name']);
					} else {
					if (isset($this->request->post['customer_message'])) {
						$data['customer_message'] = $this->request->post['customer_message'];
						} else {
