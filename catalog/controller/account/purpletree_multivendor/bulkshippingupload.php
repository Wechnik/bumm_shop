<?php
class ControllerAccountPurpletreeMultivendorBulkshippingupload extends Controller{
		private $error = array();
		public function index(){
			
			
			$this->load->language('purpletree_multivendor/bulkshippingupload');
			$this->load->model('purpletree_multivendor/bulkshippingupload');
			$this->document->setTitle($this->language->get('heading_title'));
			
			if (!$this->customer->isLogged()) {
				
				$this->session->data['redirect'] = $this->url->link('account/purpletree_multivendor/bulkshippingupload', '', true);
		...
		public function upload(){
			if(!$this->customer->validateSeller()) {
				...
				$this->response->redirect($this->url->link('account/purpletree_multivendor/bulkshippingupload', '', true));
			}
			$this->load->model('purpletree_multivendor/dashboard');
					...
					if(in_array($_FILES["import"]["type"],$mimes)){
						...
						if(array_key_exists('Shipping',$excelSheetData))
						{
							...
							if(!empty($excelSheetData['Shipping']) && $shipping){
								...
								if(!empty($fetdata)){
								$this->model_purpletree_multivendor_bulkshippingupload->deleteshippingids($postSellerId);
									foreach($fetdata as $shippingData){
										$fetchShippingData=array();
										/* $sellerIds=array();
										$ids=array();
										$ids = $this->model_purpletree_multivendor_bulkshippingupload->getSellerIds();
										if(!empty($ids)){
											$sellerIds=array_column($ids,'id');	
										} */					
										
										$countryId = $this->model_purpletree_multivendor_bulkshippingupload->getCountryId($shippingData['shipping_country']);
										...
										if($validate) {
												...
												$this->model_purpletree_multivendor_bulkshippingupload->addShipping($fetchShippingData);
						...
						$url='';
						$this->response->redirect($this->url->link('account/purpletree_multivendor/bulkshippingupload', '' . $url, true));
						} else {
		...
		public function exportExcelData($objPHPexl,$tabName,$tblName,$seller_id,$sheetIndex){
			...
			$tbl=$this->model_purpletree_multivendor_bulkshippingupload->getTableName($tblName);
			array_shift($tbl);
			//array_shift($tbl);
			...
			$expData=$this->model_purpletree_multivendor_bulkshippingupload->getExportData($tblName,$seller_id);
			$exportData=array();
			if(!empty($expData)){
				foreach($expData as $key => $value){
					//$StoreName=$this->model_purpletree_multivendor_bulkshippingupload->getStoreName($value['seller_id']);
					$country=$this->model_purpletree_multivendor_bulkshippingupload->getCountry($value['shipping_country']);
					$exportData[]=array(
					//'id'				=>	$value['id'],
		...
		public function getData(){	
			...
			'href' => $this->url->link('purpletree_multivendor/bulkshippingupload', '' . $url, true)
			);
			$this->load->model('purpletree_multivendor/dashboard');
			...
			$data['text_bulk_shipping_export'] = $this->language->get('text_bulk_shipping_export_data');
			...
			$data['export'] = $this->url->link('account/purpletree_multivendor/bulkshippingupload/export', '', true);	
			$data['upload'] = $this->url->link('account/purpletree_multivendor/bulkshippingupload/upload', '', true);	
			...
			$data['seller_id']=$this->model_purpletree_multivendor_bulkshippingupload->getAllSellerId();
