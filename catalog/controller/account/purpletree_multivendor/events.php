<?php
class ControllerAccountPurpletreeMultivendorEvents extends Controller {
		...
		$assign_area = array();
			...
			foreach($output as $key => $value){
			    $seller_detail = $this->model_purpletree_multivendor_sellerproduct->getSellername($value['product_id']);
				if($seller_detail){
		...
		public function getFilterProduct(&$route, &$args, &$output) {		
		   if($this->config->get('purpletree_multivendor_status')){		if($this->config->get('purpletree_multivendor_hyperlocal_status')){
		if((isset($this->session->data['seller_area'])&& ($this->session->data['seller_area'] > 0))){	
		$current_area = '';	
		$assign_area = array();
		     $filter_products = array();
			...
			$this->load->model('purpletree_multivendor/vendor');
			if(!empty($output)){
			   $seller_detail = $this->model_purpletree_multivendor_sellerproduct->getSellername($output['product_id']);
				if($seller_detail){
				$seller_detailss = $this->model_purpletree_multivendor_vendor->getStoreDetail($seller_detail['seller_id']);
				    $current_area = $this->session->data['seller_area'];
					if($seller_detailss['store_area'] != ''){
	...
	public function beforeheader(&$route, &$data, &$output) {
		...
		$this->load->language('purpletree_multivendor/sellerregister');
          ...
          $data['text_hyper_delivering'] = $this->language->get('text_hyper_delivering'); 
          $data['text_hyper_select'] = $this->language->get('text_hyper_select'); 
          $data['text_hyper_selects'] = $this->language->get('text_hyper_selects'); 
            if(!isset($this->session->data['seller_area'])) {		
			 ...
			 if($this->config->get('purpletree_multivendor_area_status')){
			...
			if($this->session->data['seller_area'] > 0){
		        $seller_area = $this->model_purpletree_multivendor_vendor->getSellerAreasName($this->session->data['seller_area']); 
             ...
             // start hyper local  section//
                $hyper_local=$this->config->get('purpletree_multivendor_hyperlocal_status');
       ...
       // start pop up heading 
	      if($this->config->get('purpletree_multivendor_hp_heading')){
            $popupheading=$this->config->get('purpletree_multivendor_hp_heading');			
            $data['hp_heading']=$popupheading;
		  }else{		    
			 $data['hp_heading']="";
		  }
		  if($this->config->get('purpletree_multivendor_area_status')){
		      $data['area_status_check'] = $this->config->get('purpletree_multivendor_area_status');
			  }
		  ...
		  // end pop up heading       
		    ...
		    $assign_area = array();
		    $all_seller_store = array();
			$results = $this->model_purpletree_multivendor_vendor->getSellerAreas();
			$sellerareas = array();
			if(!empty($results)){
			foreach ($results as $result) {	
			/* $all_seller_store = $this->model_purpletree_multivendor_vendor->getAllSellerInfo();
