<?php
class ControllerAccountPurpletreeMultivendorSellerorder extends Controller{
		private $error = array();
		
		public function index(){
			if (!$this->customer->isLogged()) {
				$this->session->data['redirect'] = $this->url->link('account/purpletree_multivendor/sellerorder', '', true);
			...
			$this->load->model('purpletree_multivendor/sellerorder');
			...
			'href' => $this->url->link('account/purpletree_multivendor/sellerorder', $url, true)
			);
			...
			$order_total = $this->model_purpletree_multivendor_sellerorder->getTotalSellerOrders($filter_data);
			
			$results = $this->model_purpletree_multivendor_sellerorder->getSellerOrders($filter_data);
			$orderstatus = 0;
			...
			foreach ($results as $result) {
				$total = 0;
				$product_totals  = $this->model_purpletree_multivendor_sellerorder->getSellerOrdersTotal($seller_id,$result['order_id']);
				...
				$product_commission  = $this->model_purpletree_multivendor_sellerorder->getSellerOrdersCommissionTotal($result['order_id'],$seller_id);
				...
				'admin_order_status'  => $result['admin_order_status'] ? $result['admin_order_status'] : $this->language->get('text_missing'),
				'seen'      => $result['seen'],
				...
				'view'          => $this->url->link('account/purpletree_multivendor/sellerorder/seller_order_info', 'order_id=' . $result['order_id'] . $url, true)
				);
			}
			...
			$pagination->url = $this->url->link('account/purpletree_multivendor/sellerorder','store_id=' . $sellerstore['id'] . $url . '&page={page}', true);
		...
		$store_detail = $this->customer->isSeller();
			...
			if(!$this->customer->validateSeller()) {
				$this->load->language('purpletree_multivendor/ptsmultivendor');
				$this->session->data['error_warning'] = $this->language->get('error_license');
				$this->response->redirect($this->url->link('account/purpletree_multivendor/sellerorder', '', true));
			}
			...
			$data['manage_order'] = $this->config->get('purpletree_multivendor_seller_manage_order');
			
			$order_info = $this->model_purpletree_multivendor_sellerorder->getOrder($order_id,$seller_id);
			...
			$data['breadcrumbs'] = array();
				...
				$this->model_purpletree_multivendor_sellerorder->updateSeenOrders($this->request->get['order_id'],$this->customer->getId());
				$data['shipping'] = $this->url->link('account/purpletree_multivendor/sellerorder/shipping', 'order_id=' . (int)$this->request->get['order_id'], true);
				$data['invoice'] = $this->url->link('account/purpletree_multivendor/sellerorder/invoice', 'order_id=' . (int)$this->request->get['order_id'], true);
				...
				$seller_store = $this->model_purpletree_multivendor_vendor->getStoreDetail($seller_id);
				...
				$data['store_url'] = $this->url->link('account/purpletree_multivendor/sellerstore/storeview','seller_store_id='.$seller_store['id'],true);
				...
				$customer_group_info = $this->model_purpletree_multivendor_customer_group->getCustomerGroup($order_info['customer_group_id']);
				...
				$products = $this->model_purpletree_multivendor_sellerorder->getSellerOrderProducts($this->request->get['order_id'],$this->customer->getId());
				...
				foreach ($products as $product) {
					...
					$options = $this->model_purpletree_multivendor_sellerorder->getOrderOptions($this->request->get['order_id'], $product['order_product_id']);
				...
				$vouchers = $this->model_purpletree_multivendor_sellerorder->getOrderVouchers($this->request->get['order_id']);
				...
				$orderslisttt = $this->model_purpletree_multivendor_sellerorder->getsellerorderunique($this->request->get['order_id'],$this->customer->getId());
				
				if($orderslisttt) {
					$totals = $this->model_purpletree_multivendor_sellerorder->getOrderTotals($this->request->get['order_id'],$this->customer->getId());
					} else {
				...
				$data['reward_total'] = $this->model_purpletree_multivendor_sellerorder->getTotalCustomerRewardsByOrderId($this->request->get['order_id']);
				...
				if($this->config->get('purpletree_multivendor_allow_order_status')){
			...
			foreach($data['allow_order_statuse'] as $key=>$value){
				...
				// Custom Fields
				$this->load->model('purpletree_multivendor/custom_field');
				...
				$custom_fields = $this->model_purpletree_multivendor_custom_field->getCustomFields($filter_data);
				
				foreach ($custom_fields as $custom_field) {
					if ($custom_field['location'] == 'account' && isset($order_info['custom_field'][$custom_field['custom_field_id']])) {
						if ($custom_field['type'] == 'select' || $custom_field['type'] == 'radio') {
							$custom_field_value_info = $this->model_purpletree_multivendor_custom_field->getCustomFieldValue($order_info['custom_field'][$custom_field['custom_field_id']]);
						...
						if ($custom_field['type'] == 'checkbox' && is_array($order_info['custom_field'][$custom_field['custom_field_id']])) {
							foreach ($order_info['custom_field'][$custom_field['custom_field_id']] as $custom_field_value_id) {
								$custom_field_value_info = $this->model_purpletree_multivendor_custom_field->getCustomFieldValue($custom_field_value_id);
				...
				foreach ($custom_fields as $custom_field) {
					if ($custom_field['location'] == 'address' && isset($order_info['payment_custom_field'][$custom_field['custom_field_id']])) {
						if ($custom_field['type'] == 'select' || $custom_field['type'] == 'radio') {
							$custom_field_value_info = $this->model_purpletree_multivendor_custom_field->getCustomFieldValue($order_info['payment_custom_field'][$custom_field['custom_field_id']]);
				...
				foreach ($custom_fields as $custom_field) {
					if ($custom_field['location'] == 'address' && isset($order_info['shipping_custom_field'][$custom_field['custom_field_id']])) {
						if ($custom_field['type'] == 'select' || $custom_field['type'] == 'radio') {
							$custom_field_value_info = $this->model_purpletree_multivendor_custom_field->getCustomFieldValue($order_info['shipping_custom_field'][$custom_field['custom_field_id']]);
			...
			$this->response->setOutput($this->load->view('account/purpletree_multivendor/order_info.tpl', $data));
		}
			...
			foreach ($orders as $order_id) {
				...
				if ($order_info) {
					...
					if($this->config->get('purpletree_multivendor_seller_invoice')){
						$this->load->model('purpletree_multivendor/vendor');
						$seller_store = $this->model_purpletree_multivendor_vendor->getStoreDetail($seller_id);
						$cus_seller_email  = $this->model_purpletree_multivendor_vendor->getCustomerEmailId($seller_id);
					...
					$products = $this->model_purpletree_multivendor_sellerorder->getSellerOrderProducts($order_id,$seller_id);
					...
					foreach ($products as $product) {
						...
						$options = $this->model_purpletree_multivendor_sellerorder->getOrderOptions($order_id, $product['order_product_id']);
					...
					$vouchers = $this->model_purpletree_multivendor_sellerorder->getOrderVouchers($order_id);
					...
					$orderslisttt = $this->model_purpletree_multivendor_sellerorder->getsellerorderunique($order_id,$this->customer->getId());
					if($orderslisttt) {
						$totals = $this->model_purpletree_multivendor_sellerorder->getOrderTotals($order_id,$seller_id);
						} else {
						$this->load->model('account/order');
					...
					foreach ($products as $product) {
						...
						if ($product_info) {
							...
							foreach ($options as $option) {
								...
								$product_option_value_info = $this->model_purpletree_multivendor_sellerorder->getProductOptionValue($product['product_id'], $option['product_option_value_id']);
		...
		public function history() {
			...
			if ($order_info) {
				$this->model_purpletree_multivendor_sellerorder->addOrderHistory($order_id,$seller_id, $this->request->post['order_status_id'], $this->request->post['comment'], $this->request->post['notify'], $this->request->post['override']);
		...
		public function historylist() {
			...
			$results = $this->model_purpletree_multivendor_sellerorder->getOrderHistories($this->request->get['order_id'],$this->customer->getId(), ($page - 1) * 10, 10);
			...
			$history_total = $this->model_purpletree_multivendor_sellerorder->getTotalOrderHistories($this->request->get['order_id'],$this->customer->getId());
			...
			$pagination->url = $this->url->link('account/purpletree_multivendor/sellerorder/historylist', $url . '&page={page}',true);
			...
			if ($page == 1) {
				$this->document->addLink($this->url->link('account/purpletree_multivendor/sellerorder/historylist', '', true), 'canonical');
				} elseif ($page == 2) {
				$this->document->addLink($this->url->link('account/purpletree_multivendor/sellerorder/historylist', '', true), 'prev');
				} else {
				$this->document->addLink($this->url->link('account/purpletree_multivendor/sellerorder/historylist', 'page='. ($page - 1), true), 'prev');
			}
			
			if ($limit && ceil($history_total / $limit) > $page) {
				$this->document->addLink($this->url->link('account/purpletree_multivendor/sellerorder/historylist', 'page='. ($page + 1), true), 'next');
			}
