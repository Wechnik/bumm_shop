<?php
class ControllerAccountPurpletreeMultivendorSellers extends Controller {
		public function index() {
			...
			'href' => $this->url->link('account/purpletree_multivendor/sellers', $url,true)
			);
			...
			$seller_liststotalss = $this->model_purpletree_multivendor_sellers->getSellerstotal($filter_data_seller);
			...
			foreach ($seller_liststotalss as $seller_list11) {
				if($this->config->get('purpletree_multivendor_subscription_plans')){
					...
					$getSsellerplanStatus11 = $this->model_purpletree_multivendor_sellers->getSsellerplanStatus($seller_list11['seller_id']);
					$invoiceStatus11 = $this->model_purpletree_multivendor_sellers->getInvoiceStatus($seller_list11['seller_id']);
			...
			foreach ($seller_lists as $seller_list) {
				...
				if($this->config->get('purpletree_multivendor_subscription_plans')){
					...
					$getSsellerplanStatus = $this->model_purpletree_multivendor_sellers->getSsellerplanStatus($seller_list['seller_id']);
					$invoiceStatus = $this->model_purpletree_multivendor_sellers->getInvoiceStatus($seller_list['seller_id']);
				...
				$template_product_total = $this->model_purpletree_multivendor_sellers->getTemplateProduct($filter_data);
				...
				$resultsTemplate = $this->model_purpletree_multivendor_sellers->getTemplateProducts($filter_data);
				if(!empty($results)){
				...
				$state_name = $this->model_purpletree_multivendor_vendor->getStateName($seller_list['store_state'],$seller_list['store_country']);
				$data['sellers'][] = array(
				...
				'href'        => $this->url->link('account/purpletree_multivendor/sellerstore/storeview', 'seller_store_id=' . $seller_list['id'],true),
				'seller_contact' => $this->url->link('account/purpletree_multivendor/sellercontact/customerreply', 'seller_id=' . $seller_list['seller_id'],true),
			...
			if($seller_totals ==0) {
				$data['error_warning'] = $this->language->get('text_seller_not_available');
			}
			$url = '';
			...
			'value' => 'seller-ASC',
			'href'  => $this->url->link('account/purpletree_multivendor/sellers', 'sort=seller&order=ASC' . $url,true)
			);
			
			$data['sorts'][] = array(
			'text'  => $this->language->get('text_name_desc'),
			'value' => 'seller-DESC',
			'href'  => $this->url->link('account/purpletree_multivendor/sellers','sort=seller&order=DESC' . $url,true)
			);
			...
			foreach($limits as $value) {
				...
				'text'  => $value,
				'value' => $value,
				'href'  => $this->url->link('account/purpletree_multivendor/sellers', $url . '&limit=' . $value,true)
				);
			}
			...
			$pagination->url = $this->url->link('account/purpletree_multivendor/sellers',$url . '&page={page}',true);
			if(NULL !== $this->config->get('purpletree_multivendor_store_address')){
						
					$data['pts_check_store_address'] = $this->config->get('purpletree_multivendor_store_address') ;
				}
