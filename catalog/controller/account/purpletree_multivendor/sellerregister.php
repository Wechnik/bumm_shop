<?php
class ControllerAccountPurpletreeMultivendorSellerregister extends Controller {
		private $error = array(); 
		...
		public function index() {	
			...
			if ($this->customer->isLogged()) {
				...
				$store_detail = $this->model_purpletree_multivendor_vendor->isSeller($this->customer->getId());	
				if($store_detail){
		...
		$this->document->addStyle('catalog/view/javascript/jquery/datetimepicker/bootstrap-datetimepicker.min.css');
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
				...
				$this->load->model('purpletree_multivendor/vendor');
				$file = '';
				$seller_id = $this->model_purpletree_multivendor_vendor->addSeller($customer_id,$store_name ,$file);
				if($customer_id){
					$this->subscribePlan($customer_id);
				}
				$store_id = $this->model_purpletree_multivendor_vendor->getStoreId($customer_id);
				$this->model_purpletree_multivendor_vendor->editStore($store_id, $this->request->post,$file);
				$this->model_purpletree_multivendor_vendor->editStoreImage($store_id,$store_logo,$store_banner);				
				////Start seller mail////	
				$data = $emaildata;
				...
				if (isset($customer_group_info['approval'])) {
					if($this->config->get('module_purpletree_multivendor_seller_approval') == 1){
						...
						$login_url = $this->url->link('account/purpletree_multivendor/sellerlogin', '', true);
						...
						$email_subject = $this->model_purpletree_multivendor_vendor->getmsgfromarray($replacevarsub,$subtemplatefromdb);
			...
			///Help code///	$data['helplink'] = "sfdsfsd";if (defined ('DISABLED_PTS_HELP')){if(DISABLED_PTS_HELP == 0){$data['helpcheck'] = 1;}else{$data['helpcheck'] = 0;}}else{$data['helpcheck'] = 1;}if ($this->request->server['HTTPS']) {$data['helpimage'] = HTTPS_SERVER . 'image/catalog/help.png'; } else {$data['helpimage'] = HTTP_SERVER . 'image/catalog/help.png';}/// End Help code///$data['breadcrumbs'] = array();
			...
			'href' => $this->url->link('account/purpletree_multivendor/sellerregister', '', true)
			);
			$this->load->model('purpletree_multivendor/vendor');
			...
			$data['action'] = $this->url->link('account/purpletree_multivendor/sellerregister', '', true);
			$data['sellerlogin'] = $this->url->link('account/purpletree_multivendor/sellerlogin', '', true);
			...
			$custom_fields = $this->model_purpletree_multivendor_vendor->getCustomFields();
		...
		private function validate() {
			$this->load->model('purpletree_multivendor/vendor');
			$store_info1 = $this->model_purpletree_multivendor_vendor->getStoreNameByStoreName($this->request->post['store_name']);
			...
			$custom_fields = $this->model_purpletree_multivendor_vendor->getCustomFields($customer_group_id);
		...
		public function subscribePlan($customer_id) {
			...
			$data=array();
			$plan_id=$this->model_purpletree_multivendor_subscriptionplan->defaultPlan();
			if($plan_id){	
				...
				if($startt_when == 1) {
					...
					if($this->config->get('purpletree_multivendor_multiple_subscription_plan_active')){
						...
						$current_plan_end_date=($current_plan_start_date1['new_end_date']!='0000-00-00 00:00:00')?date('m/d/Y',strtotime($current_plan_start_date1['new_end_date'])):date('m/d/Y', strtotime($current_plan_start_date1['start_date']. ' + '.$validity.' days'));
						
						} else {
						$current_plan_end_date=($current_plan_start_date1['end_date']!='0000-00-00 00:00:00')?date('m/d/Y',strtotime($current_plan_start_date1['end_date'])):date('m/d/Y', strtotime($current_plan_start_date1['start_date']. ' + '.$validity.' days'));	
					}
				...
				$start=($result['start_date']!='0000-00-00 00:00:00')?date('d/m/Y',strtotime($result['start_date'])):'';
				$end=($result['end_date']!='0000-00-00 00:00:00')?date('d/m/Y',strtotime($result['end_date'])):date('d/m/Y', strtotime($result['start_date']. ' + '.$result['validity'].' days'));
				$customer = $this->model_purpletree_multivendor_subscriptionplan->getCustomer($customer_id);
				...
				if (($this->request->server['REQUEST_METHOD'] == 'POST') /* && $this->validateForm() */) {
					...
					$sellerExist=$this->model_purpletree_multivendor_subscriptionplan->SellerExist($customer_id);
					$this->load->language('purpletree_multivendor/subscriptionplan');
					//if plan free or grand total less then zero
					if($total<=0){
						$this->model_purpletree_multivendor_subscriptionplan->enableSellerSubscription($customer_id);
					}
					//end
					
					if(!$sellerExist){
					$sellerExist=$this->model_purpletree_multivendor_subscriptionplan->addFirstSellerPlan($customer_id);					
