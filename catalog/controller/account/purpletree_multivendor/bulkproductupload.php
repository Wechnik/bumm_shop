<?php
class ControllerAccountPurpletreeMultivendorBulkproductupload extends Controller{
		private $error = array();
		public $status_msgg = array();
		public function index(){
			if (!$this->customer->isLogged()) {
				
				$this->session->data['redirect'] = $this->url->link('account/purpletree_multivendor/bulkproductupload', '', true);
		...
		public function export(){
			...
			if(!$this->customer->validateSeller()) {
				...
				$this->response->redirect($this->url->link('account/purpletree_multivendor/bulkproductupload', '', true));
		...
		public function exportLinksData($objPHPexl,$tabName,$seller_id,$language,$sheetIndex){	
			...
			$dd['E']='related';
			$dd['F']='store';
			$fff = $this->model_purpletree_multivendor_bulkproductupload->getExportData('product',$seller_id,$language);
			if(!empty($fff)&& $fff != NULL){
				$exportData=array_column($fff,'product_id');
			}
							...
							if(!empty($fetchData[$value_product_id])) {
								...
								if(!empty($categories)){
									foreach($categories as $categoryid){
										$category_info = $this->model_purpletree_multivendor_bulkproductupload->getCategory($categoryid,$this->request->post['language']);
							...
							if(!empty($fetchData[$value_product_id])) {
								...
								if(!empty($filterexp)){
									foreach($filterexp as $filterId){
										$filter_info = $this->model_purpletree_multivendor_bulkproductupload->getFilter($filterId);
								...
								if(!empty($productrelated['related_id'])){
									...
									if(!empty($relatedProduct)){
										foreach($relatedProduct as $relatedP){
											$related_info = $this->model_purpletree_multivendor_bulkproductupload->getProduct($relatedP,$this->request->post['language']);
		...
		public function upload($commonData){
			...
			if($this->config->get('purpletree_multivendor_subscription_plans')){
				...
				if($this->config->get('purpletree_multivendor_multiple_subscription_plan_active')){
					$total_plan_Product = $this->model_purpletree_multivendor_bulkproductupload->getNoOfProductForMultiplePlan($this->customer->getId());
					} else {
					$total_plan_Product = $this->model_purpletree_multivendor_bulkproductupload->getNoOfProduct($this->customer->getId());
				}
				...
				$getSsellerplanStatus = $this->model_purpletree_multivendor_bulkproductupload->getSsellerplanStatus($this->customer->getId());
													...
													*/
													...
													if($addProduct_action==='YES' ){
														...
														if($this->config->get('purpletree_multivendor_subscription_plans')){
															...
															$total_store_Product = $this->model_purpletree_multivendor_bulkproductupload->sellerTotalProduct($this->customer->getId());
														...
														if(isset($this->request->post['dataoverwrite'])==true)
														{
															if(isset($filter_data['product_id']) && $this->customer->getId()){
																//$product_check=$this->model_purpletree_multivendor_bulkproductupload->getInProductId($filter_data['product_id'],$this->customer->getId());	
																if(!empty($commonData['column_name'])){
																	
																$product_id=$this->model_purpletree_multivendor_bulkproductupload->getExistingProductId($commonData['column_name'],$filter_data[$commonData['column_name']],$this->customer->getId());
										...
										$valueseller['product_id']=$this->model_purpletree_multivendor_bulkproductupload->checkExistingProductIds($commonData['column_name'],$filterProData[$valueseller['product_id']][$commonData['column_name']]);	
												...
												if(!in_array($valueseller['product_id'],$failed_array)) {
													...
													if(in_array($product_id,$passed_array)) {
														...
														'product_id'	=>		$product_id,
														'is_approved'	=>		($this->config->get('purpletree_multivendor_product_approval'))?'0':'1'
														);
														try {
															if(is_numeric($product_id) && $product_id !='') {
																if (array_key_exists($product_id_old,$new_products)) {
...
$status_msgg[] = array("status"=>"success", "msg" => "Product  ID ".$product_id." product option value Data Updated Successfully");																	} else {
						...
						$url='';
						$this->response->redirect($this->url->link('account/purpletree_multivendor/bulkproductupload', '' . $url, true));
						} else {
		...
		protected function getData() {
			...
			'href' => $this->url->link('purpletree_multivendor/bulkproductupload',  '', true)
			);
			$this->load->model('purpletree_multivendor/dashboard');
			...
			$data['seller_bulk_product_upload_sheet'] = $this->language->get('seller_bulk_product_upload_sheet');
			...
			$data['add_product'] = $this->url->link('account/purpletree_multivendor/bulkproductupload/addproduct', '', true);	
			$data['update_product'] = $this->url->link('account/purpletree_multivendor/bulkproductupload/updateproduct', '', true);	
			$data['export'] = $this->url->link('account/purpletree_multivendor/bulkproductupload/export', '', true);
			...
			$data['lang']=$this->model_purpletree_multivendor_bulkproductupload->getAllLanguage();
