<?php
class ControllerAccountPurpletreeMultivendorBlogPost extends Controller{
		private $error = array();
		...
		public function index(){ 
			...
			$this->load->model('purpletree_multivendor/blog_post');
			...
			$post_info = $this->model_purpletree_multivendor_blog_post->getPost($blog_post_id);
			...
			if ($post_info) {
				...
				'href' => $this->url->link('account/purpletree_multivendor/blog_post', 'blog_post_id=' .  $blog_post_id,true)
				);
				...
				/******************************* Post Comments ****************************************/
				$post_comments = $this->model_purpletree_multivendor_blog_post->getPostComments($blog_post_id);
				...
				$data['action'] = $this->url->link('account/purpletree_multivendor/blog_post/addcomment', 'blog_post_id=' . $blog_post_id,true);
				...
				$results = $this->model_purpletree_multivendor_blog_post->getPopularBlog(5);
				...
				if ($results) {
					foreach ($results as $result) {
						...
						if($result['vacation'] != 1){
							...
							'title'        => $result['title'],
							'date'     => date('d M'),
							'href'        => $this->url->link('account/purpletree_multivendor/blog_post', 'blog_post_id=' . $result['blog_post_id'],'',true)
							);
						}
					}
				}
				...
				$data['pts_tags'] = array();
				$results = $this->model_purpletree_multivendor_blog_post->getPopularTags();
				if ($results) {
					foreach ($results as $result) {
		...
		public function allblog(){    
			...
			'href' => $this->url->link('account/purpletree_multivendor/blog_post/allblog','', true)
			);		
			...
			$blog_total = $this->model_purpletree_multivendor_blog_post->getTotalBlog($filter_data);		
			
			$results = $this->model_purpletree_multivendor_blog_post->getBlog($filter_data);
			...
			if ($results) {
				...
				foreach($results as $result) {								  
					if ($result['image']) {
						$image = $this->model_tool_image->resize($result['image'], $this->config->get('purpletree_sellerblog_width'), $this->config->get('purpletree_sellerblog_height'));
						} else {
						$image = $this->model_tool_image->resize('placeholder.png', $this->config->get('purpletree_sellerblog_width'), $this->config->get('purpletree_sellerblog_height'));
					}
		...
		public function addcomment(){
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) { 
				$this->model_purpletree_multivendor_blog_post->addComment($this->request->get['blog_post_id'],$this->request->post);
				
				$this->session->data['success'] = $this->language->get('text_comment_add');
				
				$this->response->redirect($this->url->link('account/purpletree_multivendor/blog_post', 'blog_post_id='.$this->request->get['blog_post_id'], true));
			}
