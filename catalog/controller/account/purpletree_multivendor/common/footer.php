<?php
class ControllerAccountPurpletreeMultivendorCommonFooter extends Controller {
	public function index() {
		...
		$data['seller_chat'] = '';
			if(NULL !== $this->config->get('purpletree_multivendor_status')){
			if($this->config->get('purpletree_multivendor_status')){
				if(NULL !== $this->config->get('purpletree_multivendor_allow_live_chat')) {
				if($this->config->get('purpletree_multivendor_allow_live_chat')) {	
