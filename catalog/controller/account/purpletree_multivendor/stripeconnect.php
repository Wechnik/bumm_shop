<?php
class ControllerAccountPurpletreeMultivendorStripeconnect extends Controller {
	public function index() {
		...
		if(isset($result->stripe_user_id)){
			...
			/* $num_of_acc= $this->model_purpletree_multivendor_stripeconnect->checkAccountExist($data['account_id']);
			if($num_of_acc){
			...
			$res='';
			//if(!$num_of_acc){
			$res= $this->model_purpletree_multivendor_stripeconnect->insertStripeAccount($data);
			//}
			if($res){
		...
		$this->response->redirect($this->url->link('account/purpletree_multivendor/dashboardicons'));
	}	
}
?>