<?php
class ControllerAccountPurpletreeMultivendorSubscriptionplan extends Controller {
		...
		public function index() {
			...
			$this->load->model('purpletree_multivendor/subscriptionplan');	
			$this->getPlan();	
		}
		...
		public function add() {
			...
			$current_plan=$this->model_purpletree_multivendor_subscriptionplan->getPlan($seller_id);
			if($startt_when == 1) {
				
				$current_plan_start_date=$this->model_purpletree_multivendor_subscriptionplan->getCurrentPlanByPlanId($seller_id,$plan_id);
				
				$current_plan_start_date1=$this->model_purpletree_multivendor_subscriptionplan->getLastPlan($seller_id,$plan_id);
				$validity=$this->model_purpletree_multivendor_subscriptionplan->validity($plan_id);
			...
			$data['end_date']='';
			$old_invoice_id=$this->model_purpletree_multivendor_subscriptionplan->getInvoiceId($seller_id);
			$result=$this->model_purpletree_multivendor_subscriptionplan->getSubscribePlanInfo($plan_id);
			$currentplan=$this->model_purpletree_multivendor_subscriptionplan->getCurrentPlan($seller_id);
			...
			$current_invoice=$this->model_purpletree_multivendor_subscriptionplan->getSellerCurrentPlan($seller_id);
			$curr_invoice=array();
			...
			$customer = $this->model_purpletree_multivendor_subscriptionplan->getCustomer($this->customer->getId());
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') /* && $this->validateForm() */) {
				...
				$invoice_id=$this->model_purpletree_multivendor_subscriptionplan->addSellerMultiplePlan($data);
				
				$sellerExist=$this->model_purpletree_multivendor_subscriptionplan->SellerExist($this->customer->getId());
				$this->load->language('purpletree_multivendor/subscriptionplan');
				
				//if plan free or grand total less then zero
				if($total<=0){
					$this->model_purpletree_multivendor_subscriptionplan->enableSellerSubscription($this->customer->getId());
				}
				//end
				if(!$sellerExist){
					$sellerExist=$this->model_purpletree_multivendor_subscriptionplan->addFirstSellerPlan($this->customer->getId());					
				   ...
				   $register_template = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate($email_code);
				   ...
				   $messtemplatefromdb = $register_template['new_message'];			
				    ...
				    $email_message = $this->model_purpletree_multivendor_vendor->getmsgfromarray($replacevar,$messtemplatefromdb);
										
					$reciver = $customer['email'];
					$this->model_purpletree_multivendor_vendor->ptsSendMail($reciver,$email_subject,$email_message);
				//end
				...
				if(!$sellerExist){					
				   ...
				   $messtemplatefromdb = $register_template['new_message'];			
					...
					$this->model_purpletree_multivendor_vendor->ptsSendMail($reciver,$email_subject,$email_message);
					//end mail
				$this->response->redirect($this->url->link('account/purpletree_multivendor/subscriptionplan/invoice', '' . 'invoice_id=' . $invoice_id .'&old_invoice_id='.$old_invoice_id. $url, true));
			}
			$this->getplan();
		}
		public function addSellerPaymentComment() {
			...
			$this->load->model('purpletree_multivendor/subscriptionplan');
			if (($this->request->server['REQUEST_METHOD'] == 'POST')  && $this->validateForm() ) {
				$invoice_id=$this->model_purpletree_multivendor_subscriptionplan->addSellerPaymentHistory($this->request->post);
				$customer = $this->model_purpletree_multivendor_subscriptionplan->getCustomer($this->customer->getId());
				$customer = $this->model_purpletree_multivendor_subscriptionplan->getCustomer($this->customer->getId());
				$seller_comment = '';
				...
				$url ='';
				$this->response->redirect($this->url->link('account/purpletree_multivendor/subscriptionplan/invoice', '' . 'invoice_id=' . $this->request->post['invoice_id'] .'&old_invoice_id='.$this->request->get['invoice_id']. $url, true));
			}
			$this->getplan();
		}
		public function paymentOffline(){ 
			...
			'href' => $this->url->link('account/purpletree_multivendor/subscriptionplan', $url, true)
			);
			...
			$data['action']=$this->url->link('account/purpletree_multivendor/subscriptionplan/addSellerPaymentComment', '' . 'invoice_id=' . $invoice_id .'&old_invoice_id='.$old_invoice_id. $url, true);
		...
		public function notify(){ 
			...
			if (strcmp ($res, "VERIFIED") == 0) {
				...
				try {
					...
					$this->model_purpletree_multivendor_subscriptionplan->addSellerPaymentHistoryfrompaypal($dataarraypaypal);
					} catch(Exception $e){ 
		...
		public function invoice(){ 
			...
			$old_invoice_id = $this->model_purpletree_multivendor_subscriptionplan->getoldinvoiceId($seller_id,$invoice_id);
			
			$data=array();
			//paypal data
			$data['return_url'] = $this->url->link('account/purpletree_multivendor/subscriptionplan/invoice/', '&invoice_id='.$invoice_id.'&paymentfrom=paypal', true);
			$data['notify_url'] = $this->url->link('account/purpletree_multivendor/subscriptionplan/notify', '', true);
			$data['origina_seller_name'] = $this->model_purpletree_multivendor_subscriptionplan->getsellernamefromsell($seller_id);
			
			$data['currency_currency'] = $this->session->data['currency'];
			$data['currency_currency1'] = (NULL != $this->config->get('purpletree_multivendor_paypal_currency'))?$this->config->get('purpletree_multivendor_paypal_currency'):$this->config->get('config_currency');
			
			$data['currency_currency'] = $this->session->data['currency'];
			$data['purpletree_multivendor_paypal_email'] = (NULL != $this->config->get('purpletree_multivendor_paypal_email') && $this->config->get('purpletree_multivendor_paypal_email') != '')?$this->config->get('purpletree_multivendor_paypal_email'):'';
		...
		//Help code///	
			...
			if(isset($invoice_id)){
				$data['invoice_data']=$this->model_purpletree_multivendor_subscriptionplan->getPlanId($invoice_id,$old_invoice_id);
			}
			$data['invoice']=array();
			...
			$seller_store=$this->model_purpletree_multivendor_subscriptionplan->getStoreDetail($this->customer->getId());
            $this->load->model('purpletree_multivendor/vendor');
	        $cus_seller_email  = $this->model_purpletree_multivendor_vendor->getCustomerEmailId($this->customer->getId());
			...
			if(isset($data['invoice_data']['invoice']['plan_id'])){
				$result=$this->model_purpletree_multivendor_subscriptionplan->getSubscribePlanInfo($data['invoice_data']['invoice']['plan_id']);
			}
			
			
			
			if(!empty($result)){
				$pts_date= $this->model_purpletree_multivendor_subscriptionplan->getCurrentPlanByPlanId1($invoice_id);
				$data['newplan'] = array(
				...
				'subscribe'        =>$this->url->link('account/purpletree_multivendor/dashboard', '' . 'plan_id=' . $result['plan_id'] . $url, true)
				
				);
			}
			$data['payment_history']=array();			
			$data['payment_history']=$this->model_purpletree_multivendor_subscriptionplan->getInvoiceHistory($invoice_id);	
			$data['payment_offline']=$this->url->link('account/purpletree_multivendor/subscriptionplan/paymentOffline', '' . 'invoice_id=' . $invoice_id .'&old_invoice_id='.$old_invoice_id. $url, true);
			
			$data['payment_online']=$this->url->link('account/purpletree_multivendor/subscriptionplan/paymentOffline', '' . 'invoice_id=' . $invoice_id . $url, true);
		...
		public function subscribe(){ 
			...
			if(!isset($this->request->get['plan_id'])){
				$this->response->redirect($this->url->link('account/purpletree_multivendor/subscriptionplan', '', true));	
			}
			...
			//$invoice_status=$this->model_purpletree_multivendor_subscriptionplan->getInvoiceStatus($this->customer->getId());
			$invoice_status=$this->model_purpletree_multivendor_subscriptionplan->getPlanInvoiceStatus($this->customer->getId(),$this->request->get['plan_id']);
			...
			if($this->config->get('purpletree_multivendor_multiple_subscription_plan_active')){		
				...
				$data['totals']['plan'][]=array(
				'sort_order'=>1,
				'code'=>$this->language->get('text_joining_fee'),
				'title'=>$this->language->get('text_joining_fee'),
				'value'=>$this->currency->format($joining_fee, $this->session->data['currency'])
				);
			}
			...
			if(isset($currentplan)){
				...
				if($this->config->get('purpletree_multivendor_multiple_subscription_plan_active')){
					$data['planid']=$this->model_purpletree_multivendor_subscriptionplan->sellerMultiplePlanId($this->customer->getId());
					} else {
					$data['planid']=$this->model_purpletree_multivendor_subscriptionplan->sellerPlanId($this->customer->getId());
				}
				if(!empty($result)){			
					$data['newplan'] = array(
					'plan_id'        => $result['plan_id']
			...
			$data['action'] = $this->url->link('account/purpletree_multivendor/subscriptionplan/add', '', true);
			$data['seller_id']=$seller_id;
			$data['plan_id']=$plan_id;
		...
		public function getPlan(){ 
			...
			if($this->config->get('purpletree_multivendor_multiple_subscription_plan_active')){
				$data['multiple_subscription_plan']=$this->config->get('purpletree_multivendor_multiple_subscription_plan_active');	
			}
			
			
			$data['seller_status']=$this->model_purpletree_multivendor_subscriptionplan->SellerPlanStatus($this->customer->getId());
			
			$plan=$this->model_purpletree_multivendor_subscriptionplan->getPlan($this->customer->getId());
			// echo "<pre>";
			// print_r($plan);
			// die;
			$data['subscription_status']=$this->model_purpletree_multivendor_subscriptionplan->getSubscriptionStauts($this->customer->getId());
			
			$total_seller_assign_product=$this->model_purpletree_multivendor_subscriptionplan->getTotalSellerPorduct($this->customer->getId());
			
			$data['seller_used_product']=$total_seller_assign_product;
			
			//$invoice_stauts=$this->model_purpletree_multivendor_subscriptionplan->getInvoiceStauts($this->customer->getId(),$plan['plan_id']);	
			
			$invoic_id=$this->model_purpletree_multivendor_subscriptionplan->getInvoiceId($this->customer->getId());
			
			$invoice_status1=$this->model_purpletree_multivendor_subscriptionplan->invoiceStauts($invoic_id);
			
			$subscription_status=$this->model_purpletree_multivendor_subscriptionplan->getSubscriptionStauts($this->customer->getId());
			
			$total_featured_product = $this->model_purpletree_multivendor_subscriptionplan->sellerTotalFeaturedProduct($this->customer->getId());
			
			$total_category_featured_product = $this->model_purpletree_multivendor_subscriptionplan->sellerTotalCategpryFeaturedProduct($this->customer->getId());
			...
			if($plan){	
				...
				'featured_store'=>$plan['featured_store']?$text_yes:$text_no				
			...
			$payment_status = $this->model_purpletree_multivendor_subscriptionplan->getstatuslist();
			
			$results=$this->model_purpletree_multivendor_subscriptionplan->getSubscriptionPlan();
			//$invoice_status=$this->model_purpletree_multivendor_subscriptionplan->getInvoiceStatus($this->customer->getId());
			
			$invoice_status=$this->model_purpletree_multivendor_subscriptionplan->getInvoiceStatus($this->customer->getId());
			if(isset($results)){
				foreach ($results as $result) {
					$invoice_status=$this->model_purpletree_multivendor_subscriptionplan->getPlanInvoiceStatus($this->customer->getId(),$result['plan_id']);
					//$this->getstausfromid($invoice['status_id']);
					if($invoice_status ==2 or $invoice_status ==NULL){
						$no_of_product=$result['no_of_product'];
						if($this->config->get('purpletree_multivendor_multiple_subscription_plan_active')){
							$no_of_plan_product=$this->model_purpletree_multivendor_subscriptionplan->getNoOfPlanProduct($this->customer->getId());
						...
						$link=$this->url->link('account/purpletree_multivendor/subscriptionplan/subscribe', 'plan_id='.$result['plan_id'].'&status='.$pts_sts, true);
						} else {
						$this->session->data['error_warning'] =$this->language->get('error_subscription_plan_pending');
						//unset($this->session->data['error_warning']);
						$link=$this->url->link('account/purpletree_multivendor/subscriptionplan','', true);	
					}
					...
					if($this->config->get('purpletree_multivendor_multiple_subscription_plan_active')){
						...
						if(!empty($data['planid'])){
							...
							if(in_array($result['plan_id'],$plan_ids)){
								$subscriptionactive=$this->model_purpletree_multivendor_subscriptionplan->sellerSubscriptionStatus($this->customer->getId());
								$subscriptionPlanStatus=$this->model_purpletree_multivendor_subscriptionplan->getMultipleSubscriptionInvoiceStatus($this->customer->getId(),$result['plan_id']);
								if($subscriptionactive && $subscriptionPlanStatus==2){
									$pts_active= $this->language->get('text_active1');
									$active_sts=1;
									} else {
									$pts_active= $this->language->get('text_inactive');
									$active_sts=1;
								}
								} else {
								$pts_active='';
						...
						if($result['plan_id']===$data['planid']){
							...
							$subscriptionPlanStatus=$this->model_purpletree_multivendor_subscriptionplan->getSubscriptionInvoiceStatus($this->customer->getId(),$result['plan_id']);
					...
					'invoice'=>$this->url->link('account/purpletree_multivendor/subscriptions&filter_plan_id='.$result['plan_id'], '', true)
