<?php
class ControllerAccountPurpletreeMultivendorSubscriptioncron extends Controller {
		...
		public function index() {
			...
			$this->load->model('purpletree_multivendor/subscriptioncron');
			...
			if($this->config->get('purpletree_multivendor_status')==1){
				...
				if($this->config->get('purpletree_multivendor_subscription_plans')==1){
					...
					if($this->config->get('purpletree_multivendor_multiple_subscription_plan_active')){
							$logger->write("Subscriptioncron if");
						$this->reminderForMultiplePlan();
						
						$this->multipleEnable();
						
						} else {
							$logger->write("Subscriptioncron else");
						$this->reminder();
		...
		protected function reminder(){
			...
			$seller_data=$this->model_purpletree_multivendor_subscriptioncron->cronReminder();
			
			
			
			if($this->config->get('purpletree_multivendor_reminder_one_days')>0){
				
				if($this->config->get('purpletree_multivendor_reminder_one_days')){
					
					$first_reminder=(int)$this->config->get('purpletree_multivendor_reminder_one_days');
							...
							foreach($seller_data as $value){
								...
								if($date_diff==0){
									...
									if($reminder < 1){
										...
										$this->model_purpletree_multivendor_subscriptioncron->updateReminder1($value['seller_id'],$reminder);
										$customer = $this->model_purpletree_multivendor_subscriptioncron->getCustomer($value['seller_id']);
										...
										$invoice_status=$this->model_purpletree_multivendor_subscriptionplan->getInvoiceStatus($value['seller_id']);
			...
			if($this->config->get('purpletree_multivendor_reminder_two_days')>0){
				
				if($this->config->get('purpletree_multivendor_reminder_two_days')){
					
					$second_reminder=(int)$this->config->get('purpletree_multivendor_reminder_two_days');
							...
							foreach($seller_data as $value){
								...
								if($date_diff==0){
									...
									if($reminder < 1){
										...
										$this->model_purpletree_multivendor_subscriptioncron->updateReminder2($value['seller_id'],$reminder);
			...
			if($this->config->get('purpletree_multivendor_reminder_three_days')>0){
				
				if($this->config->get('purpletree_multivendor_reminder_three_days')){
					
					$third_reminder=(int)$this->config->get('purpletree_multivendor_reminder_three_days');
							...
							foreach($seller_data as $value){
								...
								if($date_diff==0){
									...
									if($reminder < 1){
										...
										$this->model_purpletree_multivendor_subscriptioncron->updateReminder3($value['seller_id'],$reminder);
		...
		protected function enable(){
			...
			$seller_data=$this->model_purpletree_multivendor_subscriptioncron->planActive();
			...
			if(isset($seller_data)){
				...
				foreach($seller_data as $value){
					...
					$expire_date=($value['end_date']!='0000-00-00 00:00:00')?date('Y-m-d',strtotime($value['end_date'])):date('Y-m-d H:i:s', strtotime($value['start_date'].' + '.$value['validity'].' days'));
					...
					if($value['status'] == '1'){
						...
						if($end_date <= $today_date){	
							...
							$this->model_purpletree_multivendor_subscriptioncron->planExpired($value['seller_id'],$value['id']);
							
							$disableProduct=$this->model_purpletree_multivendor_subscriptioncron->sellerCheckData($value['seller_id'],$value['id'],$value['plan_id']);
					...
					if($start_date<=$today_date and $today_date<=$expire_date){										
						...
						$this->model_purpletree_multivendor_subscriptioncron->planEnable($value['seller_id'],$value['id'] );
						
						$this->model_purpletree_multivendor_subscriptioncron->planDisable($value['seller_id'],$value['id']);
						...
						if($ddd==1){
							...
							if($disableProduct){
								...
								$this->model_purpletree_multivendor_subscriptioncron->SellerDisableProduct($value['id']);
								
								$this->model_purpletree_multivendor_subscriptioncron->SellerDisableFeaturedProduct($value['id']);
								
								$this->model_purpletree_multivendor_subscriptioncron->SellerDisableCategoryFeaturedProduct($value['id']);
								
							}
							
							//$this->model_purpletree_multivendor_subscriptioncron->productDisable($value['seller_id'],$value['id']);
			...
			$seller_d=array();
			$seller_d=$this->model_purpletree_multivendor_subscriptioncron->sellerPlanData();
			if(!empty($seller_d)){
				foreach($seller_d as $seller_k=>$seller_result){
					if($this->model_purpletree_multivendor_subscriptioncron->activeSellerPlan($seller_result['seller_id'])){
						$this->model_purpletree_multivendor_subscriptioncron->disableSellerSubscription($seller_result['seller_id']);					
					}
				}
			}				
		...
		protected function renewPlan($plan_id,$seller_id) {
			...
			$current_plan=$this->model_purpletree_multivendor_subscriptioncron->getPlan($seller_id);
			...
			$old_invoice_id=$this->model_purpletree_multivendor_subscriptioncron->getInvoiceId($seller_id);	
			
			$result=$this->model_purpletree_multivendor_subscriptioncron->getSubscribePlan($plan_id);
			
			$currentplan=$this->model_purpletree_multivendor_subscriptioncron->getCurrentPlan($seller_id);	
			...
			$current_invoice=$this->model_purpletree_multivendor_subscriptioncron->getSellerCurrentPlan($seller_id);
			...
			$customer = $this->model_purpletree_multivendor_subscriptioncron->getCustomer($seller_id);
			...
			$invoice_id=$this->model_purpletree_multivendor_subscriptioncron->addSellerPlan($data);
			
			$sellerExist=$this->model_purpletree_multivendor_subscriptioncron->SellerExist($seller_id);
			...
			if($total<=0){
				...
				$this->model_purpletree_multivendor_subscriptioncron->enableSellerSubscription($seller_id);
		...
		protected function reminderForMultiplePlan(){
			...
			$seller_data=$this->model_purpletree_multivendor_subscriptioncron->cronReminderForMultiplePlan();
							...
							foreach($seller_data as $value){
								...
								if($date_diff==0){
									...
									if($reminder < 1){
										...
										$this->model_purpletree_multivendor_subscriptioncron->updateMultipleReminder1($value['id'],$reminder);
										$customer = $this->model_purpletree_multivendor_subscriptioncron->getCustomer($value['seller_id']);
										$this->load->model('purpletree_multivendor/vendor');
										// Seller Mail
							...
							foreach($seller_data as $value){
								...
								if($date_diff==0){
									...
									if($reminder < 1){
										...
										$this->model_purpletree_multivendor_subscriptioncron->updateMultipleReminder2($value['id'],$reminder);
										$customer = $this->model_purpletree_multivendor_subscriptioncron->getCustomer($value['seller_id']);
										$this->load->model('purpletree_multivendor/vendor');
							...
							foreach($seller_data as $value){
								...
								if($date_diff==0){
									...
									if($reminder < 1){
										...
										$this->model_purpletree_multivendor_subscriptioncron->updateMultipleReminder3($value['id'],$reminder);
		...
		protected function multipleEnable(){	
			...
			$seller_data=$this->model_purpletree_multivendor_subscriptioncron->multiplePlanActive();
			...
			if(isset($seller_data)){
				...
				foreach($seller_data as $value){
					...
					if($value['new_status'] == '1'){
						...
						if($end_date <= $today_date){
							...
							$this->model_purpletree_multivendor_subscriptioncron->planExpiredForMultiplePlan($value['seller_id'],$value['id']);
					...
					if($start_date<=$today_date and $today_date<=$expire_date){
						...
						$this->model_purpletree_multivendor_subscriptioncron->planEnableForMultiplePlan($value['seller_id'],$value['id']);
				...
				//$this->model_purpletree_multivendor_subscriptioncron->productDisableForMultiplePlan();
		...
		protected function renewMultiplePlan($plan_id,$seller_id) {
			...
			$current_plan=$this->model_purpletree_multivendor_subscriptioncron->getMultiplePlan($seller_id);
			...
			$old_invoice_id=$this->model_purpletree_multivendor_subscriptioncron->getInvoiceIdForMultiplePlan($seller_id);
			
			$result=$this->model_purpletree_multivendor_subscriptioncron->getSubscribeMultiplePlan($plan_id);
			
			$currentplan=$this->model_purpletree_multivendor_subscriptioncron->getCurrentMultiplePlan($seller_id);
			...
			$current_invoice=$this->model_purpletree_multivendor_subscriptioncron->getSellerCurrentMultiplePlan($seller_id);
			...
			$invoice_id=$this->model_purpletree_multivendor_subscriptioncron->addSellerMultiplePlan($data);
