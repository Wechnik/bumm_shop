<?php
class ControllerAccountPurpletreeMultivendorSellerstore extends Controller{
		private $error = array();
		
		public function index(){
			if (!$this->customer->isLogged()) {
				
				$this->session->data['redirect'] = $this->url->link('account/purpletree_multivendor/sellerstore', '', true);
		    ...
		    //seller area //
				...
				if($this->request->post['vacation']==1){		
					$productstts = $this->model_purpletree_multivendor_vendor->getSellerProduct($this->customer->getId());
					if(!empty($productstts)){
						foreach ($productstts as $productstt) {
							$this->model_purpletree_multivendor_vendor->updateVacationProduct($productstt['product_id'],$productstt['status'],$this->customer->getId());
							
							$this->model_purpletree_multivendor_vendor->updateProductAccrVacation($productstt['product_id']);
						}
					}
					}else{
					$productsttss = $this->model_purpletree_multivendor_vendor->getSellerProductBystatus($this->customer->getId());
					if(!empty($productsttss)){
						foreach ($productsttss as $productstts) {
							$this->model_purpletree_multivendor_vendor->updateProductAccrVacationn($productstts['product_id']);
						}
						$this->model_purpletree_multivendor_vendor->updateVacationProductByOff($this->customer->getId());
					}
				}
				///vacation
				$this->model_purpletree_multivendor_vendor->storeTime($store_id, $this->request->post);
				$this->model_purpletree_multivendor_vendor->addHoliday($store_id, $this->request->post);
			///vacation
				$this->model_purpletree_multivendor_vendor->editStore($store_id, $this->request->post,$file);
				
				$this->session->data['success'] = $this->language->get('text_success');
				
				$this->response->redirect($this->url->link('account/purpletree_multivendor/sellerstore','',true));
			}
			//Help code///	
			$data['helplink'] = "https://www.purpletreesoftware.com/knowledgebase/tag/opencart-multivendor-multi-seller-store";
			...
			'href' => $this->url->link('account/purpletree_multivendor/sellerstore', '', true)
			);
		...
		/// vacation
			...
			$data['entry_storeimage_desc'] = $this->language->get('entry_storeimage_desc');
			...
			$data['entry_youtube_link'] = $this->language->get('entry_youtube_link');
			$data['entry_google_map'] = $this->language->get('entry_google_map');
			$data['entry_store_image'] = $this->language->get('entry_store_image');
			...
			if (isset($this->error['store_description'])) {
				$data['error_storedescription'] = $this->error['store_description'];
				} else {
				$data['error_storedescription'] = '';
			}
			...
			$data['action'] = $this->url->link('account/purpletree_multivendor/sellerstore', '', true);
			...
			if (isset($store_id) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
				...
				$seller_info_social = $this->model_purpletree_multivendor_vendor->getStoreSocial($store_id);
			    /// vacation
			$store_time = array();
			$store_holiday = array();
			 $store_time = $this->model_purpletree_multivendor_vendor->getStoreTime($store_id);
			 $store_holiday = $this->model_purpletree_multivendor_vendor->getStoreHoliday($store_id);
			/// vacation
			}
			/// vacation
			...
			$area_info = $this->model_purpletree_multivendor_vendor->getSellerAreaByID($area_id);
	    ...
	    // seller area	
			...
			if (isset($this->request->post['store_banner']) && is_file(DIR_IMAGE . $this->request->post['store_banner'])) {
				...
				$data['banner_thumb'] = $this->model_tool_image->resize('purpletree_banner.jpg', 100, 100);
			}
		...
		$custom_fields = $this->model_purpletree_multivendor_vendor->getCustomFieldsForSeller($filter_data);

		foreach ($custom_fields as $custom_field) {
			$data['custom_fields'][] = array(
				'custom_field_id'    => $custom_field['custom_field_id'],
				'custom_field_value' => $this->model_purpletree_multivendor_vendor->getCustomFieldValues($custom_field['custom_field_id']),
		...
		$customer_info = $this->model_purpletree_multivendor_vendor->getCustomer($this->customer->getId());
		...
		public function becomeseller(){
			...
			/* if($isSeller){
				...
				$data['action'] = $this->url->link('account/purpletree_multivendor/sellerstore/reseller', '', true);
				$data['is_removed'] = 1;
				}
				} else {
				$data['action'] = $this->url->link('account/purpletree_multivendor/sellerstore/becomeseller', '', true);
				$data['is_removed'] = 0;
			} */
			...
			if($isSeller){
				...
				if($isSeller['is_removed']){
					...
					$this->response->redirect($this->url->link('account/purpletree_multivendor/dashboardicons','',true));
				}
				} else {
				$this->response->redirect($this->url->link('account/purpletree_multivendor/sellerregister','',true));
			}
			...
			if (isset($this->request->post['seller_storename'])) {
				$data['store_name'] = $this->request->post['seller_storename'];
				} else {
				$data['store_name'] = '';
			}
		...
		public function storeview() {
			...
			$data['error_warning'] = '';
			$this->load->model('purpletree_multivendor/vendor');
			if($this->request->get['seller_store_id']){
				$seller_Store = $this->model_purpletree_multivendor_vendor->getStore($this->request->get['seller_store_id']);
				// hyper local 
			$current_area = '';
			$seller_area_find = 0;
			$assign_area = array();
			if($this->config->get('purpletree_multivendor_hyperlocal_status')){
			...
			$data['text_store_timings'] = $this->language->get('text_store_timings');
			...
			$data['button_wishlist'] = $this->language->get('button_wishlist');
			$data['button_compare'] = $this->language->get('button_compare');
			$data['purpletree_multivendor_store_social_link'] = $this->config->get('purpletree_multivendor_store_social_link');///Social links
			...
			if(isset($this->request->get['p_url'])){
				$data['breadcrumbs'][] = array(
					...
					'href' => $this->url->link('account/purpletree_multivendor/sellerstore/storeview&seller_store_id='.$sellerstore.'&p_url='.$this->request->get['p_url'], '', true)
				);
		    }else{			
			$data['breadcrumbs'][] = array(
			'text' => $this->language->get('text_storeview'),
			'href' => $this->url->link('account/purpletree_multivendor/sellerstore/storeview&seller_store_id='.$sellerstore, '', true)
			);
		}
			$store_detail = $this->model_purpletree_multivendor_vendor->getStore($sellerstore);
			$seller_info_social = $this->model_purpletree_multivendor_vendor->getStoreSocial($sellerstore);
			//social Links
			...
			if($store_detail  and ($store_detail['store_status']==1)){
				$seller_detailss = $this->model_purpletree_multivendor_vendor->getStoreDetail($store_detail['seller_id']);
			...
			if($store_detail   and ($store_detail['store_status']==1)){
				...
				$data['store_rating'] = $this->model_purpletree_multivendor_vendor->getStoreRating($store_detail['seller_id']);
				$this->load->model('purpletree_multivendor/vendor');
				$cus_seller_email  = $this->model_purpletree_multivendor_vendor->getCustomerEmailId($store_detail['seller_id']);
				...
				$data['store_state'] = $this->model_purpletree_multivendor_vendor->getStateName($store_detail['store_state'],$store_detail['store_country']);			
				$data['store_country'] = $this->model_purpletree_multivendor_vendor->getCountryName($store_detail['store_country']).',';
				...
				$data['seller_review_status'] = $this->config->get('purpletree_multivendor_seller_review');
				$data['store_review'] = $this->url->link('account/purpletree_multivendor/sellerstore/sellerreview','seller_id=' . $store_detail['seller_id'], true);
				
				$data['store_shipping_policy'] = $this->url->link('account/purpletree_multivendor/sellerstore/storedesc','path=shippingpolicy'. '&seller_store_id=' . $store_detail['id'], true);
				
				$data['store_return_policy'] = $this->url->link('account/purpletree_multivendor/sellerstore/storedesc','path=returnpolicy'. '&seller_store_id=' . $store_detail['id'], true);
				
				$data['store_about'] = $this->url->link('account/purpletree_multivendor/sellerstore/storedesc','path=aboutstore'. '&seller_store_id=' . $store_detail['id'], true);
				
				$data['seller_contact'] = $this->url->link('account/purpletree_multivendor/sellercontact/customerreply','seller_id=' . $store_detail['seller_id'], true);
				...
				if (is_file(DIR_IMAGE . $store_detail['store_banner'])) {
					if($this->config->get('purpletree_multivendor_storepage_layout')){
						$data['store_banner'] = $this->model_tool_image->resize($store_detail['store_banner'], 652, 267);
						} else {	
						$data['store_banner'] = $this->model_tool_image->resize($store_detail['store_banner'], 900,300);
					}
					} else {
					if($this->config->get('purpletree_multivendor_storepage_layout')){
						$data['store_banner'] = $this->model_tool_image->resize('catalog/purpletree_banner.jpg', 652, 267);
						} else {
				...
				if (is_file(DIR_IMAGE . $store_detail['store_image'])) {
					$data['store_image'] = $this->model_tool_image->resize($store_detail['store_image'], 473, 290);
					} else {
					$data['store_image'] = $this->model_tool_image->resize('catalog/purpletree_banner.jpg', 473, 290);
				}			
				...
				$data['allCategory']=$this->url->link('account/purpletree_multivendor/sellerstore/storeview', '&seller_store_id='.$seller_store_id, true);
				
				$categoryMenu =$this->model_purpletree_multivendor_sellerproduct->categoryMenu($seller_store_id);
				
				$categoryMenuProduct =$this->model_purpletree_multivendor_sellerproduct->categoryMenuProduct($seller_store_id,$p_url);
				...
				if(!empty($categoryMenu)){
					foreach($categoryMenu as $category_key=>$category_value){
						if($category_value['category_id']){
							...
							'url'=>$this->url->link('account/purpletree_multivendor/sellerstore/storeview', '&seller_store_id='.$seller_store_id.'&p_url='.$category_value['category_id'], true),
...
// Hyper local
		...
		if(isset($templateproducts)){
			foreach($templateproducts as $templateproduct){
				...
				$product_specials = $this->model_purpletree_multivendor_sellerproduct->getProductSpecials($templateproduct['product_id']);
				$special = false;
		...
		if(!empty($templateproductss)){
			$template_product=1;
				...
				$seller_products = $this->model_purpletree_multivendor_vendor->getAllSellerProducts($store_detail);
				...
				$toatl_seller_products = $this->model_purpletree_multivendor_vendor->getAllTotalSellerProducts($store_detail);
				
				
				$data['seller_products'] = $this->load->controller('common/product_card', array('products' => $seller_products, 'page' => 'seller'));
				
//				echo '<pre>'; print_r($data['seller_products']); die;
...
//							$image = $this->model_tool_image->resize($seller_product['image'], $this->config->get($this->config->get('config_theme') . '_image_product_width'), $this->config->get($this->config->get('config_theme') . '_image_product_height'));
//							} else {
...
//				$this->load->model('purpletree_multivendor/vendor');
//				$data['show_seller_name'] = $this->config->get('purpletree_multivendor_show_seller_name');
//				$data['multivendor_status'] = $this->config->get('purpletree_multivendor_status');
//                $data['show_seller_address'] = $this->config->get('purpletree_multivendor_show_seller_address');
//					$seller_detail = array();
//					$pts_quick_status = '';
//					$seller_detail = $this->model_purpletree_multivendor_sellerproduct->getSellername($seller_product['product_id']);
//
...
//					   $cuntry_name = $this->model_purpletree_multivendor_vendor->getStoreDetail($seller_detail['seller_id']);
//					   $cuntry_name = $this->model_purpletree_multivendor_vendor->getCountryName($seller_detailss['store_country']);
//					   $state_name = $this->model_purpletree_multivendor_vendor->getStateName($seller_detailss['store_state'],$seller_detailss['store_country']);
//					   $store_address = $seller_detailss['store_address'].','.$seller_detailss['store_city'].','.$state_name.','.$cuntry_name;
//					   $seller_link  = $this->url->link('account/purpletree_multivendor/sellerstore/storeview', 'seller_store_id=' . $seller_detailss['id']);
...
//				        'quick_order'       => $this->url->link('account/purpletree_multivendor/quick_order', '&product_id='.$seller_product['product_id'].$url, true),
//
...
//						'description' => utf8_substr(strip_tags(html_entity_decode($seller_product['description'], ENT_QUOTES, 'UTF-8')), 0, $this->config->get($this->config->get('config_theme') . '_product_description_length')) . '..'
//						);
//						$countttt++;
//
//					}
//				}
		...
		// End Hyper local 
				if(isset($this->request->get['seller_store_id'])){
					$seller_storessst = $this->model_purpletree_multivendor_vendor->checkSellerVacation($this->request->get['seller_store_id']);
				...
				'href'  => $this->url->link('account/purpletree_multivendor/sellerstore/storeview','&sort=p.sort_order&order=ASC' . $url.'&seller_store_id='.$sellerstore,true)
				);
				
				$data['sorts'][] = array(
				'text'  => $this->language->get('text_name_asc'),
				'value' => 'pd.name-ASC',
				'href'  => $this->url->link('account/purpletree_multivendor/sellerstore/storeview', '&sort=pd.name&order=ASC' . $url.'&seller_store_id='.$sellerstore,true)
				);
				
				$data['sorts'][] = array(
				'text'  => $this->language->get('text_name_desc'),
				'value' => 'pd.name-DESC',
				'href'  => $this->url->link('account/purpletree_multivendor/sellerstore/storeview', '&sort=pd.name&order=DESC' . $url.'&seller_store_id='.$sellerstore,true)
				);
				
				$data['sorts'][] = array(
				'text'  => $this->language->get('text_price_asc'),
				'value' => 'p.price-ASC',
				'href'  => $this->url->link('account/purpletree_multivendor/sellerstore/storeview',  '&sort=p.price&order=ASC' . $url.'&seller_store_id='.$sellerstore,true)
				);
				
				$data['sorts'][] = array(
				'text'  => $this->language->get('text_price_desc'),
				'value' => 'p.price-DESC',
				'href'  => $this->url->link('account/purpletree_multivendor/sellerstore/storeview', '&sort=p.price&order=DESC' . $url.'&seller_store_id='.$sellerstore,true)
				);
				
				$data['sorts'][] = array(
				'text'  => $this->language->get('text_model_asc'),
				'value' => 'p.model-ASC',
				'href'  => $this->url->link('account/purpletree_multivendor/sellerstore/storeview', '&sort=p.model&order=ASC' . $url.'&seller_store_id='.$sellerstore,true)
				);
				
				$data['sorts'][] = array(
				'text'  => $this->language->get('text_model_desc'),
				'value' => 'p.model-DESC',
				'href'  => $this->url->link('account/purpletree_multivendor/sellerstore/storeview', '&sort=p.model&order=DESC' . $url.'&seller_store_id='.$sellerstore,true)
				);
				...
				foreach($limits as $value) {
					...
					'text'  => $value,
					'value' => $value,
					'href'  => $this->url->link('account/purpletree_multivendor/sellerstore/storeview', $url . '&limit=' . $value.'&seller_store_id='.$sellerstore,true)
					);
				}
				...
				$pagination->url = $this->url->link('account/purpletree_multivendor/sellerstore/storeview', $url . '&page={page}',true);
				...
				if ($page == 1) {
					$this->document->addLink($this->url->link('account/purpletree_multivendor/sellerstore/storeview', '&seller_store_id='.$sellerstore, true), 'canonical');
					} elseif ($page == 2) {
					$this->document->addLink($this->url->link('account/purpletree_multivendor/sellerstore/storeview', '&seller_store_id='.$sellerstore, true), 'prev');
					} else {
					$this->document->addLink($this->url->link('account/purpletree_multivendor/sellerstore/storeview', '&seller_store_id='.$sellerstore.'&page='. ($page - 1), true), 'prev');
				}
				
				if ($limit && ceil($toatl_seller_products / $limit) > $page) {
					$this->document->addLink($this->url->link('account/purpletree_multivendor/sellerstore/storeview', '&page='. ($page + 1), true), 'next');
				}
		...
		public function storedesc() { 
			...
			if ($store_info) {
				if($this->request->get['path']=="shippingpolicy"){
					...
					'href' => $this->url->link('account/purpletree_multivendor/sellerstore/storedesc&path='.$this->request->get['path'].'&seller_store_id='.$store_id, '', true)
					);
		...
		private function validateSeller(){
			...
			if($this->request->post['become_seller']){ 
				...
				$store_info1 = $this->model_purpletree_multivendor_vendor->getStoreNameByStoreName($this->request->post['seller_storename']);	
		...
		private function validateForm(){
			...
			$store_info = $this->model_purpletree_multivendor_vendor->getStoreById($this->customer->getId());
			...
			$store_detai = $this->customer->isSeller();
			$store_info11 = 		$this->model_purpletree_multivendor_vendor->getStoreByNameId($this->request->post['store_name'],$store_detai['id']);
			if(!empty($store_info11)){
				if (isset($store_detai['id'])) {
					if (($store_detai['id'] != $store_info11['id'] && strtoupper(trim($this->request->post['store_name']))==strtoupper(trim($store_info11['store_name'])))) {
			...
			/* $EMAIL_REGEX='/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/';
				...
				$store_info1 = $this->model_purpletree_multivendor_vendor->getStoreByIdd($this->customer->getId(),$this->request->post['store_email']);
				if($store_info1){
			...
			/* if ((utf8_strlen(trim($this->request->post['store_description'])) < 3)) {
				$this->error['store_description'] = $this->language->get('error_storedescription');
			} */
		...
		public function removeseller(){
			...
			if (!$this->customer->isLogged()) {
				$this->session->data['redirect'] = $this->url->link('account/purpletree_multivendor/removeseller', '', true);
		...
		public function sellerreview() { 
			...
			if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateReview()) {
				...
				$this->response->redirect($this->url->link('account/purpletree_multivendor/sellerstore/sellerreview&seller_id='.$this->request->post['seller_id'],'',true));
			}
			...
			'href' => $this->url->link('account/purpletree_multivendor/dashboard', '', true)
			);
			...
			if(isset($this->request->get['seller_id'])){
				...
				'href' => $this->url->link('account/purpletree_multivendor/sellerstore/sellerreview&seller_id='.$this->request->get['seller_id'], '', true)
				);
				$data['seller_id'] = $seller_id;
				
				$this->load->model('purpletree_multivendor/sellerreview');
				if(!$this->model_purpletree_multivendor_sellerreview->canReview($datasend = array('seller_id' =>$seller_id,'customer_id' =>$data['customer_id']))) {
					$data['warning'] = $this->language->get('no_can_review');
				}
				
				$data['action'] = $this->url->link('account/purpletree_multivendor/sellerstore/sellerreview&seller_id='.$data['seller_id'],'',true);
				...
				$data['result_check'] = $this->model_purpletree_multivendor_sellerreview->checkReview($filter_data);
				...
				$pagination->url = $this->url->link('account/purpletree_multivendor/sellerstore/sellerreview', 'seller_id=' . $data['seller_id'] . '&page={page}',true);
		 ...
		 if ($direction=='rtl'){
			...
			$this->document->addStyle('catalog/view/theme/default/stylesheet/purpletree/custom.css'); 
				...
				if($this->customer->isSeller()){
					...
					$data['breadcrumbs'][] = array(
					'text' => $this->language->get('text_storereview'),
					'href' => $this->url->link('account/purpletree_multivendor/sellerstore/sellerreview', '', true)
					);
					$filter_data = array(
					'start'              => ($page - 1) * $limit,
					'limit'              => $limit,
					...
					$pagination->page = $page;
					$pagination->limit = $this->config->get('config_limit_admin');
					$pagination->url = $this->url->link('account/purpletree_multivendor/sellerstore/sellerreview', 'seller_id=' . $seller_id . '&page={page}',true);
		...
		protected function subscription($seller){
			...
			if($this->config->get('purpletree_multivendor_subscription_plans')){
				...
				if($this->config->get('purpletree_multivendor_multiple_subscription_plan_active')){
					...
					$invoiceStatus=array();
					$getSsellerplanStatus = $this->model_purpletree_multivendor_sellercontact->getSsellerplanStatusMultiplePlan($seller);
					$invoiceStatus = $this->model_purpletree_multivendor_sellercontact->getInvoiceStatusMultiplePlan($seller);   
					...
					$getSsellerplanStatus = $this->model_purpletree_multivendor_sellercontact->getSsellerplanStatus($seller);
					$invoiceStatus = $this->model_purpletree_multivendor_sellercontact->getInvoiceStatus($seller);
		...
		if (isset($this->request->get['filter_name'])) {
            ...
            $results = $this->model_purpletree_multivendor_vendor->getSellerAreass($filter_data);
			foreach ($results as $result) {
				$json[] = array(
