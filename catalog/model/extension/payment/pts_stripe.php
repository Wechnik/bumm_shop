<?php
	class ModelExtensionPaymentPtsStripe extends Model {
		public function getMethod($address, $total) {
			...
			$code='pts_stripe';
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('pts_stripe_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
			if($this->config->get('pts_stripe_status') == 1 ){
			if ($this->config->get('pts_stripe_total_min') < $total && $total <  $this->config->get('pts_stripe_total_max')) {
				$status = true;
				} elseif (!$this->config->get('pts_stripe_geo_zone_id')) {
				$status = true;
			...
			if($code != '') {
				$method_data = array(
				'code'       => $code,
				'title'      => $this->config->get('pts_stripe_title'),
				'terms'      => '',
				'sort_order' => $this->config->get('pts_stripe_sort_order')
				);
			}
			}
