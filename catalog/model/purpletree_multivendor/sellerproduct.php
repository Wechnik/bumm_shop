<?php
class ModelPurpletreeMultivendorSellerproduct extends Model{
	...
	////// For Sub Category ////////
		...
		////// End Sub Category ////////
		//////////For get Image manager Extension status ///////////
		
		public function getModifications() {
			$sql = "SELECT code,status FROM " . DB_PREFIX . "modification WHERE code='ProductImageManager' AND status=1";		
			$query = $this->db->query($sql);		
			if($query->num_rows){
				return 1;
				} else {
		...
		//////////////////Image manager Extension status//////////
		//////Start Image manager ///////
		//simpleStore image 
		public function getUnlikUrls($id,$value){ $query=$this->db->query("SELECT EXISTS(
		...
		public function getTotalSellerProducts($data = array()) {
			$sql = "SELECT COUNT(DISTINCT p.product_id) AS total FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) JOIN " . DB_PREFIX . "purpletree_vendor_products pvp ON(pvp.product_id=p.product_id) LEFT JOIN ".DB_PREFIX. "product_to_category ptc ON(ptc.product_id=p.product_id)";
		...
		$pts_shipping_charge = 'NULL';
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "',
			 minimum = '" . $minimum . "', subtract = '" . $subtract . "',
			  stock_status_id = '" . $stock_status_id . "', date_available = '',
			   manufacturer_id = '" . (int)$data['manufacturer_id'] . "',
			    shipping = '1', shipping_charge=".$pts_shipping_charge.", price = '" . (float)$data['price'] . "',
			      ...
			      weight_class_id = '" . (int)$data['weight_class_id'] . "',
			       `length` = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "',
			        height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "',
			         status = '1', tax_class_id = '" . $tax_class_id . "',
...
//					$this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "', tag = '" . $this->db->escape($value['tag']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
			...
			if (isset($data['keyword']) && $data['keyword']) {
				...
				foreach($data['product_description'] as $key=>$value){
					...
					$data['keyword']=$value['name'].'_'.$product_id;
					break;
				}
				$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'product_id=" . (int)$product_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
			}
		...
		public function getProductCategories($product_id) {
			...
			foreach ($query->rows as $result) {
				$product_category_data[$result['category_id']] = $result['category_id'];
			}
