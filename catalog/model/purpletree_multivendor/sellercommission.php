<?php
class ModelPurpletreeMultivendorSellercommission extends Model{
		...
		public function getCommissions($data=array()){
			...
			if(!isset($data['filter_date_from']) && !isset($data['filter_date_to'])){
				...
				$sql .= " AND DATE(pvc.created_at) >= '".$this->db->escape($end_date)."'";
				$sql .= " AND DATE(pvc.created_at) <= '".$this->db->escape(date('Y-m-d'))."'";
			}
		...
		public function getTotalsale($data=array()){
			...
			if(!empty($data['filter_date_from']) && !empty($data['filter_date_to'])){
				$end_date = date('Y-m-d', strtotime("-30 days"));
				$sql .= " AND DATE(pvo.created_at) >= '".$this->db->escape($end_date)."'";
				$sql .= " AND DATE(pvo.created_at) <= '".$this->db->escape(date('Y-m-d'))."'";
			}
			$query  = $this->db->query($sql);
