<?php
class ModelPurpletreeMultivendorSellerblogpost extends Model {
		...
		public function addPost($data,$seller_id) {
			...
			if ($data['keyword']) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'blog_post_id=" . (int)$blog_post_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
			}
		...
		public function editPost($blog_post_id, $data) {
			...
			$this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE query = 'blog_post_id=" . (int)$blog_post_id . "'");
		...
		public function getPost($blog_post_id) {
			$query = $this->db->query("SELECT DISTINCT *, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'blog_post_id=" . (int)$blog_post_id . "' LIMIT 0,1) AS keyword FROM " . DB_PREFIX . "purpletree_vendor_blog_post bp LEFT JOIN " . DB_PREFIX . "purpletree_vendor_blog_post_description bpd ON (bp.blog_post_id = bpd.blog_post_id) WHERE bp.blog_post_id = '" . (int)$blog_post_id . "' AND bpd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
