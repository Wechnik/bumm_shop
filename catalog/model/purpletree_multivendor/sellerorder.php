<?php
class ModelPurpletreeMultivendorSellerorder extends Model{
			...
			if ($order_info) { 
				...
				if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
					...
					if ($order_info['affiliate_id'] && $this->config->get('config_affiliate_auto')) {
						...
						$this->model_affiliate_affiliate->addTransaction((int)$order_info['affiliate_id'], (int)$order_info['commission'], (int)$order_id);
					}
				...
				/* foreach ($order_product_query->rows as $order_product) {
					...
					//category_commission
					elseif(!empty($seller_id['seller_id'])) {
					if($seller_id['store_commission'] > 0){
					$commission = ($order_product['total']*$seller_id['store_commission'])/100;
					} else {
					$commission = ($order_product['total']*$this->config->get('purpletree_multivendor_commission'))/100;
					}
					$this->db->query("DELETE FROM " . DB_PREFIX . "purpletree_vendor_commissions WHERE order_id = '" . (int)$order_id . "' AND product_id ='".(int)$order_product['product_id']."'");
				...
				if (in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && !in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
					...
					if ($order_info['affiliate_id']) {
						...
						$this->model_affiliate_affiliate->deleteTransaction((int)$order_id);
					}
				}
				...
				if (!$order_info['order_status_id'] && $order_status_id) {
					...
					$subject = sprintf($language->get('text_new_subject'), html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8'), (int)$order_id);
					
					// HTML Mail
					$data = array();
					
					$data['title'] = sprintf($language->get('text_new_subject'), $order_info['store_name'], (int)$order_id);
					...
					$data['text_new_product'] = $language->get('text_new_product');
					...
					$data['link'] = $order_info['store_url'] . 'index.php?route=account/order/info&order_id=' . (int)$order_id;
		...
		if($comment_ins!=''){
			if($order_info_url!=''){
				if($order_down_url!=''){
					if($p_comment!=''){
						$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_first');
						
					} else {
						$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_prodcut_mail_to_seller_condition_second');
					
					}
				} else {
					if($p_comment!=''){
						$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_third');
						
					} else {
						$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_fourth');
			...
			if($order_info_url!=''){
				if($order_down_url!=''){
					if($p_comment!=''){
						$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_nine');
						
					} else {
						$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_ten');
					
					}
				} else {
					if($p_comment!=''){
						$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_eleven');
						
					} else {
						$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_twelve');
				...
				if($order_down_url!=''){
					if($p_comment!=''){
						$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_thirteen');
						
					} else {
						$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_fourteen');
					
					}
				} else {
					if($p_comment!=''){
						$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_fifteen');
						
					} else {
						$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('seller_product_mail_to_cutomer_condition_sixteen');
		...
		$subject = $this->model_purpletree_multivendor_vendor->getmsgfromarray($mail_subject_vars,$getTemplate['new_subject']);
		$message = $this->model_purpletree_multivendor_vendor->getmsgfromarray($mail_msg_vars,$getTemplate['new_message']);

		$this->model_purpletree_multivendor_vendor->ptsSendMail($reciver,$subject,$message);
					...
					if(isset($seller_detail['email'])){
						...
						$language->load('mail/order');
						$subject = sprintf($language->get('text_new_subject'), html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8'), (int)$order_id);
			...
			$getTemplate='';	
			if($p_comment!=''){
			$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_prodcut_mail_to_seller_condition_first');		
			} else {
			$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_prodcut_mail_to_seller_condition_second');	
			}		
		...
		if($p_comment!=''){
		$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_prodcut_alert_mail_condition_first');		
		} else {
		$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_prodcut_alert_mail_condition_second');	
		}
		...
		$this->model_purpletree_multivendor_vendor->ptsSendMail($reciver,$subject,$message);
				...
				$date_added=date($language->get('date_format_short'), strtotime($order_info['date_added']));
						...
						$product_detail='';
						foreach ($order_product_query->rows as $product) {
							$product_detail .= $product['quantity'] . 'x ' . $product['name'] . ' (' . $product['model'] . ') ' . html_entity_decode($this->currency->format($product['total'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value']), ENT_NOQUOTES, 'UTF-8') . "<br>";
							...
							foreach ($order_option_query->rows as $option) {
								...
								$product_detail .= chr(9) . '-' . $option['name'] . ' ' . (utf8_strlen($value) > 20 ? utf8_substr($value, 0, 20) . '..' : $value) . "<br>";
							}
						}
						$product_desc='';
						foreach ($order_voucher_query->rows as $voucher) {
		...
		$this->model_purpletree_multivendor_vendor->ptsSendMail($reciver,$subject,$message);
					...
					if ($email && filter_var($email, FILTER_VALIDATE_EMAIL)) {
						$this->model_purpletree_multivendor_vendor->ptsSendMail($email,$subject,$message);
						}
					}
					}
				...
				if ((int)$order_info['order_status_id'] && (int)$order_status_id && $notify) {
					...
					$store_url= $this->db->escape($order_info['store_url']) . 'index.php?route=account/purpletree_multivendor/sellerstore/storeview&seller_store_id='. (int)$seller_datile['store_id'];
					$p_order_info='';
					if ($order_info['customer_id']) {
						$p_order_info .= $this->db->escape($order_info['store_url']) . 'index.php?route=account/order/info&order_id=' . (int)$order_id . "<br>";
					}
	...
	if($p_order_info!=''){		
		...
		$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_customer_notify_on_order_update_condition_first_by_seller');
		}
		
		if($p_order_status!='' && $p_comment==''){	
		$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_customer_notify_on_order_update_condition_second_by_seller');
		}
					
		if($p_order_status=='' && $p_comment!=''){
			$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_order_update_notify_condition_third_by_seller');
		}
					
		if($p_order_status=='' && $p_comment==''){
			$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_order_update_notify_condition_fourth_by_seller');
		}
	} else {
		if($p_order_status!='' && $p_comment!=''){	
		$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_customer_notify_on_order_update_condition_fifth_by_seller');
		}
		
		if($p_order_status!='' && $p_comment==''){	
		$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_customer_notify_on_order_update_condition_sixth_by_seller');
		}
					
		if($p_order_status=='' && $p_comment!=''){
			$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_customer_notify_on_order_update_condition_seventh_by_seller');
		}
		...
		$getTemplate='';			
		if($p_order_status!='' && $p_comment!=''){	
		$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_order_update_condition_first_by_seller');
		}
		if($p_order_status!='' && $p_comment==''){	
		$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_order_update_condition_second_by_seller');
		}
					
		if($p_order_status=='' && $p_comment!=''){
			$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_order_update_condition_third_by_seller');
		}
					
		if($p_order_status=='' && $p_comment==''){
			$getTemplate = $this->model_purpletree_multivendor_vendor->getSelleRegisterEmailTemplate('front_seller_order_update_condition_fourth_by_seller');
		}
			$mail_subject_vars=array(
								'_ORDER_ID_'=>$order_id,
								'_STORE_NAME_'=>$order_store_name
