<?php
class ModelPurpletreeMultivendorSubscriptioncron extends Model{
		...
		public function cronReminderForMultiplePlan() {
			$sql = "SELECT CONCAT(c.firstname, ' ', c.lastname) AS seller_name, pvpd.plan_name,pvsp.id,pvps.seller_id,pvp.no_of_product,pvsp.reminder1,pvsp.reminder2,pvsp.start_date,pvsp.new_end_date,pvsp.reminder,pvsp.new_status,pvp.validity,pvsp.plan_id FROM " . DB_PREFIX . "purpletree_vendor_seller_plan pvsp LEFT JOIN " . DB_PREFIX . "customer c ON (c.customer_id = pvsp.seller_id) LEFT JOIN " . DB_PREFIX . "purpletree_vendor_plan pvp ON (pvp.plan_id = pvsp.plan_id) LEFT JOIN " . DB_PREFIX . "purpletree_vendor_plan_subscription pvps ON (pvps.seller_id = pvsp.seller_id) LEFT JOIN " . DB_PREFIX . "purpletree_vendor_plan_description pvpd ON (pvpd.plan_id = pvsp.plan_id) WHERE pvpd.language_id='". (int)$this->config->get('config_language_id') ."' AND pvsp.new_status = 1"; 
			$query = $this->db->query($sql);
			if($query->num_rows){
				return $query->rows;
			}
		...
		public function planActive() {
			$sql = "SELECT CONCAT(c.firstname, ' ', c.lastname) AS seller_name, pvpd.plan_name,pvsp.id,pvp.no_of_product,pvsp.start_date,pvsp.end_date,pvsp.reminder,pvsp.reminder1,pvsp.reminder2,pvps.seller_id,pvsp.status,pvp.validity,pvp.plan_id FROM " . DB_PREFIX . "purpletree_vendor_seller_plan pvsp LEFT JOIN " . DB_PREFIX . "customer c ON (c.customer_id = pvsp.seller_id) LEFT JOIN " . DB_PREFIX . "purpletree_vendor_plan pvp ON (pvp.plan_id = pvsp.plan_id) LEFT JOIN " . DB_PREFIX . "purpletree_vendor_plan_subscription pvps ON (pvps.seller_id = pvsp.seller_id) LEFT JOIN " . DB_PREFIX . "purpletree_vendor_plan_description pvpd ON (pvpd.plan_id = pvsp.plan_id) WHERE pvpd.language_id='". (int)$this->config->get('config_language_id') ."'"; 
			$query = $this->db->query($sql);
			if($query->num_rows){
				return $query->rows;
			}
