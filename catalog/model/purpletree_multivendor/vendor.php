<?php
class ModelPurpletreeMultivendorVendor extends Model{
	///seller area /////
	...
	public function isSeller($customer_id){	
		...
		public function addSeller($customer_id,$store_name,$filename = ''){
			$this->db->query("INSERT into " . DB_PREFIX . "purpletree_vendor_stores SET seller_id ='".(int)$customer_id."', store_name='".$this->db->escape(trim($store_name))."', multi_store_id='".(int)($this->config->get('config_store_id'))."',store_status='".(int)(!$this->config->get('purpletree_multivendor_seller_approval'))."', store_created_at= NOW(), store_updated_at= NOW()");
		...
		public function becomeSeller($customer_id,$store_name,$filename=''){
			if($store_name['become_seller']){
				$this->db->query("INSERT into " . DB_PREFIX . "purpletree_vendor_stores SET seller_id ='".(int)$customer_id."', store_name='".$this->db->escape(trim($store_name['seller_storename']))."', document='". $this->db->escape($filename) ."',store_status='".(int)(!$this->config->get('purpletree_multivendor_seller_approval'))."', store_created_at= NOW(), store_updated_at= NOW()");
				$store_id = $this->db->getLastId();
			}
			else {
				$store_id = 0;
		...
		public function reseller($customer_id,$store_name){
			if($store_name['become_seller']){	
				$this->db->query("UPDATE " . DB_PREFIX . "purpletree_vendor_stores SET store_status='".(int)(!$this->config->get('purpletree_multivendor_seller_approval'))."', is_removed=0 WHERE seller_id='".(int)$customer_id."'");
				$store_id = 1;
			}
			else {
				$store_id = 0;
		...
		public function getStore($store_id){
			$query = $this->db->query("SELECT pvs.*,CONCAT(c.firstname, ' ', c.lastname) AS seller_name, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'seller_store_id=" . (int)$store_id . "') AS store_seo FROM " . DB_PREFIX . "purpletree_vendor_stores pvs JOIN " . DB_PREFIX . "customer c ON(c.customer_id = pvs.seller_id) where pvs.id='".(int)$store_id."'");
			return $query->row;
		}
		...
		public function editStore($store_id,$data,$file = ''){
			...
			if((empty($data['seller_area'])) ||(!isset($data['seller_area']))){
				$data['seller_area'] = "";
			}
			...
			if ($data['store_seo']) {
				...
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "url_alias WHERE query = 'seller_store_id=" . (int)$store_id . "'");
				if($query->num_rows > 0){
					$row = $query->row;
					$this->db->query("UPDATE " . DB_PREFIX . "url_alias SET query = 'seller_store_id=" . (int)$store_id . "', keyword = '" . $this->db->escape($data['store_seo']) . "' WHERE url_alias_id=".$this->db->escape($row['url_alias_id']));
					
					} else{
					$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'seller_store_id=" . (int)$store_id . "', keyword = '" . $this->db->escape($data['store_seo']) . "'");
				}
			}
		}
		...
		public function getStoreSeo($seo_url) {
			...
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "url_alias WHERE LCASE(keyword) = '".$this->db->escape(utf8_strtolower($seo_url)) . "'");
			return $query->row;
		}
		...
		public function getStoreNameByStoreName($store_name2){		
			$sql = "SELECT pvs.id ,pvs.seller_id ,pvs.store_name FROM " . DB_PREFIX . "purpletree_vendor_stores pvs WHERE pvs.store_name = '" . $this->db->escape(trim($store_name2)) . "'";
			$query = $this->db->query($sql);      
			return $query->row;	
		}
		public function getStoreByNameId($store_name2,$id){		
			$sql = "SELECT pvs.id ,pvs.seller_id ,pvs.store_name FROM " . DB_PREFIX . "purpletree_vendor_stores pvs WHERE pvs.store_name = '" . $this->db->escape(trim($store_name2)) . "' AND pvs.id !='".$id."'";
			$query = $this->db->query($sql);      
			return $query->row;	
		}
